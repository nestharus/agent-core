...
| CodeRabbit | Model - `gpt-medium` single-review owner + per-conversation fixer | Run one review generation; independently continue each finding conversation until CodeRabbit resolves its GraphQL thread, then require CodeRabbit `APPROVED` on the exact final head before merge. Do not request a follow-up review generation. |
