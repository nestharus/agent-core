...
| Proposal | Model - audit (`gpt-high`) + scope (`gpt-xhigh`) + shortcut (`gpt-xhigh`) + supported-surface (`gpt-xhigh`) + verification-plan review (`gpt-xhigh`) in parallel | The model gate reviews whether the plan is suitable to execute. It does not produce the observed experiment result required by `conventions/behavioral-proof.md`. No human gate. |
...
| CodeRabbit | Model - `gpt-medium` single-review owner + per-conversation fixer | Run one review generation; require each in-diff conversation's GraphQL thread to resolve and each scoped outside-diff finding to receive an accepted current-revision disposition, then require CodeRabbit `APPROVED` on the exact final head before merge. Detailed inventory and disposition mechanics remain in the referenced CodeRabbit contract. Do not request a follow-up review generation. |
| Test audit | Model - `gpt-high` root with independently audited `spec-alignment`, `test-quality`, and `coverage-delta` children | Validate that tests encode intended behavior first, reduce named risks, preserve fixture externality, keep explicit levels, and do not weaken ground-truth tests to match implementation. The gate is consumable only with a hash-current nested process-tree PASS and production-validator `VALID`. |
...
| Draft PR acquisition (Phase 7) | Automated (`implementation-pipeline-orchestrator` using `pr-writer`) | Create or reuse one exact-base/head draft PR before CodeRabbit; persist and verify its identity. |
| Phase 9 verified PR outcome | Automated under resolved invocation/project policy | Before ticket dispatch, freeze caller-owned backend/site/ticket/route/attempt/PR/reviewed expected context and require exact producer comment/readback equality through the production validator. Both terminal outcomes carry current expected-context/result and owned Phase 4/6/8 proof path-hashes. `auto_merge_after_phase_9=false` returns `VERIFIED_DRAFT_PR` with exact `is_draft=true` and `phase_8_reviewed_is_draft=true`. When true and authorized, the orchestrator retains immutable Phase 8 reviewed OPEN draft identity, freshly fetches both exact base/head refs, requires provider/fetched/reviewed equality for both OIDs before and after ready, and restores/proves exact OPEN draft state on every pre-merge refusal before returning to Phase 8. Restoration failure is non-replayable `BLOCKED:ready-state-restoration-failed`. The expected-head guarded merge remains defense in depth; merge invocation permits no later undo/replay and failures are `BLOCKED:merge-attempt-started`. Public-audience promotion requires tiered-approval pre-authorization; there is no Phase 10. |
