...
| Proposal | Model - audit (`gpt-high`) + scope (`gpt-high`) + shortcut (`gpt-high`) + supported-surface (`gpt-high`) + proof-risk (`gpt-high`) in parallel | The risk assessment is the review. No human gate. |
| Alignment (optional) | Model - `gpt-high` | Direction check. Returns `ALIGNED`, `MISALIGNED`, or `NEEDS_REVISION`. |
