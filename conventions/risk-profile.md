...
Non-negotiable touched-file ownership clause: risk profiles and code-quality auditors score the files/components touched by the active WU. Pre-existing code-quality debt inside touched files/components is current-WU risk: estimate it, score it, and use it as decomposition evidence when the cleanup is too large for one WU. Only findings that live exclusively outside touched files/components remain residual/context for the current WU. Apply this clause with `conventions/code-quality.md` `## Auditor Scope Boundary` and `## Touched-file ownership`.
...
| **Brittleness markers** | None of the markers below. | One or two markers. | Three or more markers, or any single marker named in `~/ai/conventions/no-deferred-stubs.md` or `no-backwards-compatibility.md` exemption notes. Markers: `# TODO` / `FIXME` / `HACK` density on the surface; recent revert commits; `Refs: <ticket>` trailers showing repeated re-work; tests with `xfail` / `skip` / `runner skip` decorators on this surface; commented-out code blocks; type-ignore comments; "this is wrong but it works" patterns. |
...
The set of axes is not a checklist to fill; it is the lens. New axes can be added as the project's failure modes accumulate. Removing axes requires a project-level decision recorded in `${worktree_path}/DECISIONS.md` or an explicitly supplied project decision log.
...
|---|---|---|
| Phase 3 (proposal) | `Forbidden behaviors`, supported-surface track, assumption register, test-intent track, net-value statement — ALL phases produce these regardless of mode. `Forbidden behaviors` is behavior-forbidding only and must not predeclare narrower work inside touched-file/component ownership. **Mode-conditional**: the test-intent track in lean mode lists the smoke-level coverage; in exhaustive mode it lists every named behavior + a fixture-source per item. | As lean, plus per-axis MEDIUM/HIGH evidence the proposal addresses, plus a residual-risk section naming what the proposal does NOT reduce outside touched-file/component ownership. |
...
- **Lean mode as "skip the phase"**. Lean is a smaller artifact, not a missing one. If a phase is genuinely not needed, that is a `${worktree_path}/DECISIONS.md` skip with justification, not a lean-mode shortcut.
