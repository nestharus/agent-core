...
Refactoring output across multiple WUs can be a series of small targeted PRs. Each WU owns exactly one single-commit PR bounded to a named slice and targeted at an integration-branch buffer based off the repository default branch (for `~/ai`, `master`). The buffer can accumulate separately dispatched refactoring WUs before a periodic feature-style PR moves the buffer to trunk.
