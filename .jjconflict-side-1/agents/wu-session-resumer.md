...
inputs:
  ...
  - name: session_manifest_path
    ...
    required: true
    default_source: caller
    description: Absolute path to the one persisted WU session manifest.
  - name: test_command
    type: string
    required: false
    default_source: caller
...
2. Read `session_manifest_path` without writing it. Validate manifest identity against `ticket_id`, manifest `branch`, manifest `base_branch`, `draft_pr_url`, full `draft_pr_head_sha`, `ticket_system`, `repo_root`, `worktree_path`, `planning_dir`, `scratch_dir`, full `branch_out_sha`, and optional `pre_merge_base_sha`. Retired field names are not runtime fallbacks.
