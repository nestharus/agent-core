...
inputs:
  ...
  - name: base_branch
    ...
    required: false
    default_source: base
    description: Parent branch for worktree creation, provenance, diff and rebase baselines, review, PR targeting, merge verification, and post-merge evidence.
...
- `base_branch` — the WU parent and ticket PR base (default `main`). Callers whose trunk is not `main`, feature coordinators, refactoring coordinators, and stacked-WU callers pass the actual parent explicitly. Once resolved, this value is authoritative for all parent-sensitive lifecycle surfaces. If an explicitly supplied base is missing or cannot be resolved, halt with `BLOCKED:invalid-base-branch`; never retry or silently substitute `main`.
