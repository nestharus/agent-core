...
For new protected-change lifecycles, this consumer selects CRW's [evidence-to-change method](/home/nes/projects/code-review-workflows/trunk/methods/evidence-to-change.md) as support for the lifecycle above, not its replacement. Explicitly select its [Corrected Observe Frame Decide Act recurrence](/home/nes/projects/code-review-workflows/trunk/methods/evidence-to-change.md#corrected-observe-frame-decide-act-recurrence), and read and apply its selection, responsibility/return and reopening contracts; CRW owns the method and role procedures. Root retains purpose, joint consequential decisions, separate isolated ad-hoc implementation, verification, delivery obligations and effect authority. This is not a seven-agent sequence or a new planning phase. Use the method's [decision context](/home/nes/projects/code-review-workflows/trunk/methods/decision-context.md) to carry authentic target **and why**, constraints, evidence and engineering freedom through existing sufficient notes, not a mandatory new artifact.
...
Deploy the corrected-cohort default only after the authoritative CRW checkout exposes the compatible, explicitly selectable complete cohort contract and method recurrence named above. Provider delivery precedes consumer activation and actual linked-source readback; the root owns that paired delivery, not this source declaration. CRW procedure availability alone does not migrate older standalone/discretionary callers or active/historical runs from their selected policy. If the required interface or observation is unavailable, return the gap and its consequences for explicit consumer disposition under applicable obligations; do not infer completion or silently substitute affected/discretionary continuation for the corrected loop. This deployment dependency does not grant synchronization or external-mutation authority.
