...
Select exactly one session profile: `$HOME/.codex`, `$HOME/.codex2`, `$HOME/.codex3`, or `$HOME/.codex4`. Every new Codex child uses `gpt-6-sol` at `xhigh` during this outage, even where an older dispatch entry names a different model, and has **all effective MCP servers disabled**, including servers supplied outside the selected profile's `config.toml`. The installed CLI merges `-c 'mcp_servers={}'` with existing server tables; that setting alone does not disable servers. The launcher discovers servers from the effective `codex mcp list --json`, passes individual disable flags, supplies the docs server transport and disable flag for the `codex exec` injection case, and requires a second effective list to report every server disabled before execution. The four profiles are session stores, **not a four-child concurrency cap**; concurrent children need distinct attempts, terminal handles, and worktrees where writing is involved.
...
1. **Observe.** For **new** protected-change lifecycles, select the [script-backed default](workflows/mutable-review.md): CRW **`perspective-lives-v1`** with **`after-act-domain-v1`**, through its [owning executable contract](/home/nes/projects/code-review-workflows/trunk/risk-axis-reviewers/workflows/corrected-cohort/README.md#new-run-consumer-default). The actual entry is `python ~/ai/workflows/mutable-review.py -- start <private-run> --file <actual-config.json>`. CRW owns configuration and semantic accounting; this is not execution of the manual domain-pass corrected loop. Activation follows the paired-delivery dependency below. New-run policy selection grants bounded domain activation only, not access, Decide, child, graph-edit, worker, secret/capture or real-runner authority. Supply those actual inputs; conflicting denials or missing required authority return to root, not fabricated grants or silent manual fallback. Explicit manual/standalone users and active/historical runs retain their selected procedures. Supply the real question, purpose, actors, environment, material and actual access/delegation authority. This selects initial read-only exposure to all nine CRW-defined families (domains) in separate investigative contexts and all 27 constituent questions, not salience-based family selection; CRW owns their definitions, necessary-child work, actual domain results and collection procedure. Collect actual source-grounded observations, causal explanations, counterevidence, uncertainty and leads, including explicit missing or partial required work. Dispatch alone, absent reports or coordinator-inline substitutes do not complete required exposure; unavailable authority remains a visible gap, not permission to expand authority or silently waive work. Follow material relationships beyond starting files or ticket boundaries within actual access authority. Focused observations, explicit single-family assignments and reviewer children remain bounded work, not new full episodes or claims of full coverage. Depth remains responsive to evidence and unknowns, without a predetermined investigation budget. A review observation is not a recommendation, correction instruction or merge decision; completed family exposure does not establish safety or recall.
...
5. **Observe consequences.** Run applicable verification and examine the change and its effects. Retain useful source-bound evidence and uncertainty, including newly discovered consequences of correction; feed those back through interpretation and decision. After each linked corrective Act, root supplies and collects Observe for **every remaining active domain** and its **enabled perspectives** under the [CRW executable contract](/home/nes/projects/code-review-workflows/trunk/risk-axis-reviewers/workflows/corrected-cohort/README.md#outcomes-and-linked-act), then returns its evidence through Frame/Decide. CRW owns cycle qualification, decision-use accounting, bounded after-Act domain judgment, interruption and run boundaries; do not substitute manual domain-pass retirement or root/Frame impact selection. Apply the complete contract rather than a local duplicate algorithm: no affected-domain substitute, unchanged confirmation sweep, or new all-nine schedule beneath focused children. Additional bounded inquiry may deepen evidence without replacing owed visits. Depth remains responsive to evidence and unknowns, without arbitrary count/time/depth limits. Explain which earlier evidence still applies to current relationships and what needs observation; reuse informs each owed current visit, not membership. Earlier retired-domain results do not certify later material; newly discovered losses still return through Frame/Decide. Only CRW-authorized activation can restore retired domains. The interim selected policy retains disabled siblings in active domains and expired automatic windows as accepted exposure, not a guarantee of small or absent late regressions; collaborative-inquiry refinement is not selected here. Later focused work or accepted uncertainty cannot retroactively complete missing initial exposure or active visits. An interrupted investigation returns partial evidence and outstanding questions, not an inferred success. Neither static checks nor absence of observations proves universal safety or stochastic efficacy.
6. **Dispose / deliver.** The root owns verification, remaining decisions, delivery obligations and final disposition. Before merge, confirm that applicable checks and decisions still describe the content being delivered; revisit affected conclusions after changes. Complete only when consequential decisions are resolved, required verification and obligations are met, and accepted risks/no-action reasons and remaining uncertainty are visible. Operational cohort stabilization is distinct from consumer delivery. When consequential decisions are resolved, no Act is warranted and required verification and delivery obligations are met, root may complete disposition with accepted findings without claiming cohort all-pass or retirement. Retain each finding, owner, reason, scope, risk and evidence; acceptance alone supplies neither pass, N/A nor retirement; the saved CRW policy governs accounting while Acts continue. Missing required work remains missing. No universal reviewer `PASS` or zero-observation count supplies merge authority. External mutation requires the authority already granted by the task and environment.
...
### Selected reasoning-and-change support

For new protected-change lifecycles, this consumer selects CRW's [evidence-to-change method](/home/nes/projects/code-review-workflows/trunk/methods/evidence-to-change.md) as support for the lifecycle above, not its replacement. Explicitly select its [Executable Observe Frame Decide Act recurrence](/home/nes/projects/code-review-workflows/trunk/methods/evidence-to-change.md#executable-observe-frame-decide-act-recurrence), and read and apply its selection, responsibility/return and reopening contracts; CRW owns the method and role procedures. Root retains purpose, joint consequential decisions, separate isolated ad-hoc implementation, verification, delivery obligations and effect authority. This is not a seven-agent sequence or a new planning phase. Use the method's [decision context](/home/nes/projects/code-review-workflows/trunk/methods/decision-context.md) to carry authentic target **and why**, constraints, evidence and engineering freedom through existing sufficient notes, not a mandatory new artifact.

Select permitted work separately from input readiness: observation-only requests stay observation-only; planning/decision support permits no source or external effects and decisions require explicit delegated authority; authorized change still requires the task's implementation/effect authority. Current sufficient evidence, agendas or authentic decisions may supply later-stage inputs directly without rerunning upstream roles. Readiness grants no permission and does not waive initial all-nine coverage, owed active-domain visits or verification. Frame (the existing interpretation and decision-framing responsibilities) and authorized Decide may concisely reaffirm an unchanged valid target and rationale against current evidence before another Act; reuse or an implementation-omission label cannot bypass that judgment or become a stale correction queue. Older discretionary callers and active/historical assignments retain their selected policy.

The selected controller collects distinct Frame and Decide conversations under their actual procedures and caller-supplied authority; root-inline notes do not substitute for its required returns or grant Decide authority. For other explicitly selected method work, root may assign interpretation, decision-framing and joint-deciding support within actual bounds; semantic support assignments use opaque `gpt-xhigh`. Name the actual root, assigned owners, collectors and output/answer destinations. Collect actual returns, not dispatch receipts: original domain and necessary-child evidence, dissent, gaps and scoped operational results remain directly recoverable within actual privacy/access authority through named collectors to root and assigned bounded Frame/authorized Decide owners, not only a collector synthesis; supported accounts and open agendas go to root or an explicitly assigned framer/authorized decider; commitments and rationale, implementation candidates and verification return to root. Apply the method's reopening routes to partial/unavailable work and unanswered questions. Root retains unassigned factual and protection questions until capable authorized assignment; future tickets are not recipients. Keep the run purpose/basis, cycle/start population, actual material/result references, linked Acts and supported retirement or new-run reasons recoverable in the selected adapter's original records and sufficient notes, not a second consumer ledger or schema. Conflicting goals, premises or authority return to root rather than silently changing decisions.

For authorized-change delivery here, root must examine whether the delivered content realizes authentic decisions **and their rationale** as part of existing verification and decision-currentness responsibilities. The dedicated [decision-realization reader](/home/nes/projects/code-review-workflows/trunk/methods/decision-realization.prompt.md) is optional: when warranted, select its actual focused procedure with current candidate, decision context, verification, authority and root's collecting destination; collect its actual output or explicit gap. Root-inline examination does not claim that reader ran. Independently supply and collect wider consequence observation under step 5 and the method, not only fidelity leads; realization does not establish absence of other losses. Realization and separately admitted protection inquiry remain bounded complementary capabilities, not substitutes for active-domain visits or membership selectors.

Compatible CRW `start-default`, executable policy and method-recurrence delivery must precede this consumer selector's activation; general method or manual-cohort availability alone is insufficient. Root must first dispose the separate runtime-compatibility inquiry and collect required consequence/design observation; candidate source and fake tests grant no merge or deployment authority. An unavailable interface or required return stays an explicit gap for root disposition under existing obligations, never inferred completion or expanded authority. Method links and static/manual checks do not qualify their own adoption, prove actual invocation or establish efficacy; root owns cross-repository pairing and order, must separately verify authorized provider delivery then consumer activation with actual linked-source readback, and must arrange any required updates to the provider's current-consumer-selection description after actual deployment. A candidate worktree or commit is not deployment. If provider changes invalidate the relied-on interfaces or evidence, return the affected relationship to root for renewed examination rather than assuming previously examined content still applies. This selection neither changes other consumers nor retrospectively qualifies active/historical work.

### Protection inquiry to existing test capabilities

This consumer uses the method's [protection selection and return route](/home/nes/projects/code-review-workflows/trunk/methods/evidence-to-change.md#selecting-and-collecting-protection-inquiry) for an authentic root-accepted material protection question. When selected, incorporate the actual [protection investigator procedure](/home/nes/projects/code-review-workflows/trunk/methods/protection-investigator.prompt.md) in an opaque `gpt-xhigh` assignment with accepting root, question and why, actors/environment, independent behavior authority, finding/source, actual gate selections/results or known absence, access bounds and root's collecting destination. This is optional bounded read-only inquiry, not another family, automatic suite audit or repeated initial review/planning step. Acceptance authorizes inquiry, not the finding's truth, product-risk re-adjudication or test authorship.

Root collects the causal account and interprets its protection needs against the authentic outcome and current conditions before deciding any next work. Reuse sufficient existing evidence/specifications without redundant investigation. The following are **selective capabilities, not a sequence or actual assignees**:

| Question / admitted need | Existing capability and return to collect |
| --- | --- |
| Independent intent is missing or ambiguous | [Behavior investigator](agents/behavior-investigator.md): supply the bounded target and existing evidence; collect its intent sources, verified behavior specification or specific unresolved questions. An accepted protection account alone is neither behavior authority nor a writer-ready spec. |
| Verified or human-confirmed behavior is already sufficiently specified and authoring is separately authorized | [Test writer](agents/test-writer.md): supply the admitted spec, target, test level and isolated worktree; collect tests, spec provenance, actual results and gaps. Expected mappings, preconditions and boundaries must come from independent intent, not the implementation or chosen mitigation. Honor infrastructure, strict expected-failure provenance and structural-verification exclusions. |
| Test quality / spec alignment needs examination | [Coverage auditor](agents/coverage-auditor.md): select the applicable task, such as `validate-new` with actual tests/specs; collect quality reasoning and limits. This is not measurement or automatic `audit-existing` / `full-audit`. |
| Coverage measurement / inventory is needed | [Coverage analyzer](agents/coverage-analyzer.md): select task and scope; collect measured data, its conditions and partial results, retaining any accompanying quality leads as distinct from measurements. |
| Uncovered-area prioritization and broader expansion are actually selected | [Coverage expansion](agents/coverage-expansion-operator.md): admit its inventory/risk/intent/authoring/report obligations and effects separately; collect its actual products and unresolved returns. Do not select it merely to write tests for an already-known behavior. |
| Selected test outcomes need verification | Use an applicable project runner/verification capability within its own authority. [Red](agents/red-phase-gate.md) / [green](agents/green-phase-gate.md) are only applicable with their actual Python/structured-runner support and admitted inputs; collect detailed selection, per-ID outcomes/transitions, conditions and limits, not just the advisory headline. |

Before any downstream assignment, read the current project wrapper/contract sidecar first and the operator body, including task admission, stop conditions and reporting obligations. Name the actual assigned actor, effect bounds and collecting destination; root collects actual returns and owns any unassigned answers. Authoring, runner execution, broad audit/expansion and external/report publication effects require separate applicable authority. Ordinary protection observation does not inherit PDF production, test authoring or CodeRabbit; selected providers retain their own obligations, not a local exemption.

A supplied failing command, node ID, CI log or structured failure is already reproduction: preserve that signal and its conditions for applicable candidate verification, without recreating it just to fill a pipeline slot. It is not automatically an admissible `RED_PHASE.md` for green. Missing history/spec provenance, incompatible runners and ambiguous provider admission or classification return to root with the concrete source conflict, affected question and useful partial evidence. Do not invent fields, baseline history or report verdicts, bypass stop conditions, repair providers here, or advertise an ambiguous route as executable. Root selects another genuinely applicable authorized capability or retains the gap under the method's reopening rules; a capability name or future ticket is not an answer recipient.

At return, preserve useful conditional/manual, skipped/xfail and narrow mechanism controls and what they actually distinguish; `DEAD` or removal-directed labels alone authorize no deletion. Keep control existence, selection and actual assertion execution distinct. No-selection green and collection recovery do not prove outcome discrimination; unchanged IDs need not mean unchanged setup. Carry actual conditions/oracles, genuine baseline limits, counterevidence and residual review dependence when automation is unreliable. Partial, interrupted or ambiguous returns remain root-owned, not verification discharge. This handoff changes no lifecycle, initial-coverage, effects, delivery or historical-evidence rules; source/manual coherence checks establish neither invocation nor empirical efficacy.
...
Deploy the script-backed default only after the authoritative CRW checkout exposes the compatible `start-default`, complete executable contract and executable method recurrence named above, and the actual AI provider/runner interface is qualified for the granted use. Provider delivery precedes consumer activation and actual linked-source readback; the root owns that paired delivery, not this source declaration. CRW procedure availability alone does not migrate older standalone/discretionary callers or active/historical runs from their selected policy. If the required interface or observation is unavailable, return the gap and its consequences for explicit consumer disposition under applicable obligations; do not infer completion or silently substitute manual or affected/discretionary continuation for the selected executable policy. This deployment dependency does not grant synchronization or external-mutation authority.
...
- Script-backed new protected-change default (explicit alternate/manual and historical selections preserved): [`~/ai/workflows/mutable-review.md`](workflows/mutable-review.md)
