...
**A1. Content revision** (`gpt-high`) — adds missing sections. Writes editing report.
...
**C1. Em dash fixes** (`gpt-high` fix → `gpt-high` reviewer)
**C2. Banned opener fixes** (`gpt-high` fix → `gpt-high` reviewer)
**C3. Triad fixes** (`gpt-high` fix → `gpt-high` reviewer)
**C4. Pseudo list fixes** (`gpt-high` fix → `gpt-high` reviewer)
**C5. Repeated opener fixes** (`gpt-high` fix → `gpt-high` reviewer)
**C6. Repeated frame fixes** (`gpt-high` fix → `gpt-high` reviewer)
**C7. Cliche fixes** (`gpt-high` fix → `gpt-high` reviewer)
**C8. Moralizing/status fixes** (`gpt-high` fix → `gpt-high` reviewer)
**C9. Performative sentence fixes** (`gpt-high` fix → `gpt-high` reviewer)
**C10. Hedging fixes** (`gpt-high` fix → `gpt-high` reviewer) — only if B2 found any
**C11. Filler/throat-clearing fixes** (`gpt-high` fix → `gpt-high` reviewer) — only if B2 found any
...
**D1. Hook placement** (`gpt-high` fix → `gpt-high` reviewer) — does the opening create tension tied to the hypothesis?
**D2. Reframe consolidation** (`gpt-high` fix → `gpt-high` reviewer) — is the reframe stated once, crisply, early?
**D3. Container section framing** (`gpt-high` fix → `gpt-high` reviewer) — do container sections have framing prose?
**D4. Border bridges** (`gpt-high` fix → `gpt-high` reviewer) — per specific border flagged in skeleton analysis
**D5. Table framing** (`gpt-high` fix → `gpt-high` reviewer) — do tables have prose making their argument explicit?

Reviewers use `gpt-high` because structural quality is judgment, not pattern-matching.

### Phase E: Flow and Cohesion (Rubric 3)

**E1. Paragraph cohesion pass** (`gpt-high` fix → `gpt-high` reviewer) — each paragraph develops one idea, known-to-new pattern.
**E2. Causality pass** (`gpt-high` fix → `gpt-high` reviewer) — every major claim has a concrete "because".
**E3. Cognitive load pass** (`gpt-high` fix → `gpt-high` reviewer) — concrete before abstract, cash out implications.

### Phase F: Adversarial Robustness (Rubric 4)

**F1. Intent alignment** (`gpt-high`) — writes private sentences (not published), disciplines scope.
**F2. Pushback pass** (`gpt-high` fix → `gpt-high` reviewer) — strongest objection addressed.
**F3. Worst case reader test** (`gpt-high` fix → `gpt-high` reviewer) — hostile reader accusation preempted or contained.
**F4. Quote test** (`gpt-high` fix → `gpt-high` reviewer) — 3 random sentences, revise any usable against intent.
**F5. Evidence toggle** (`gpt-high` fix → `gpt-high` reviewer) — every empirical claim cited, labeled hypothesis, or labeled anecdote.
...
**G2. Soft tell fixes** (`gpt-high` fix → `gpt-high` reviewer).
**G3. Tempo pass** (`gpt-high` fix → `gpt-high` reviewer) — metronomic cadence, rhythm device clusters, rationed pattern budgets.
**G4. Tightening pass** (`gpt-high` fix → `gpt-high` reviewer) — compression, tone consistency.
...
**I1. Independent quality gate** (`gpt-high` — a fresh agent that did NOT participate in any earlier stage) — runs the Final Check from WRITING_SKILL_MASTER.md plus document-type-specific checks. Returns PASS or FAIL with specific findings.
...
|---|---|---|
| Content revision | `gpt-high` | Strategic judgment, adding substantive content |
| Skeleton / scan agents | `gpt-high` | Pattern matching, high controllability |
| Mechanical ban fixes | `gpt-high` | Creative rewriting to preserve meaning |
| Mechanical fix reviewers | `gpt-high` | Pattern detection, high controllability |
| Structural fixes (D/E/F) | `gpt-high` | Writing judgment |
| Structural reviewers | `gpt-high` | Judgment, not pattern matching |
| Editorial | `gpt-high` | Voice and rhythm |
| Quality gate | `gpt-high` | Independent judgment |
