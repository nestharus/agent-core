...
- **P4 prototype-test PR publication**: after you file spawned tickets, finalize marker reasons on the prototype-test branch with real ticket keys or URLs per `~/ai/conventions/prototype-pending-tests.md`. Push the prototype-test branch. Dispatch `prototype-test-pr-writer` with `prototype_test_branch_ref`, the dossier answer, proof-test audit, spawned tickets, `test_manifest_path`, pending marker convention, implementation ticket URLs, base branch, repo root, and output path. Verify the writer produced title/body files, then run `gh pr create --draft --title "$(cat ${output_path}.title)" --body-file ${output_path}`. Capture URL evidence for the draft PR. Write the URL, branch, test paths/node IDs, marker reason format, and ticket mapping into `dossier/answer.md` and `dossier/spawned-tickets.md` without changing the existing dossier file list.
