...
- The driver owns exactly one CodeRabbit review and finding-resolution pass per PR: snapshot CodeRabbit review IDs before its trigger, resume or suppress the outstanding generation, enforce the 300-second minimum cadence between driver-owned polls, dispatch one fixer invocation per current-generation finding, preserve each structured outcome, push fixed commits, post exact-comment replies with readback, persist `single_review_completion`, and terminate without a follow-up review.
...
- `generation_result=REVIEW_COMPLETED` with `terminal_reason=approved` is CodeRabbit success. A prior-head approval is history only and cannot satisfy the exact-reviewed-head gate after a rate-limited new-head trigger.
- `terminal_reason=review_applied` is accepted completion only when the findings came from a `REVIEW_COMPLETED` generation and every finding has a validated `fixed`, `replied`, or `fixed_and_replied` disposition with all required push/reply readbacks. A finding requiring no code change is resolved by an exact rationale reply. The final head is intentionally not sent through CodeRabbit again.
