...
- Artifacts: `risk/NN-audit.md`, `risk/NN-scope.md`, `risk/NN-shortcut.md`, `risk/NN-supported-surface.md`, `risk/NN-proof-risk.md`
...
- `proof-risk`: does the proposal's `## Proof plan` map each runtime claim to a proof method whose evidence class exercises that claim. Producer: `~/ai/agents/proof-risk-auditor.md` with `mode=phase-3-proposal`. Canonical output path: `${planning_dir}/risk/${wu_lower}-proof-risk.md`.
- Rule: all five proposal-risk reports must return `LOW` on the LOW/MEDIUM/HIGH verdict dimension.
...
- Rule: the Phase 4 join manifest records a distinct `gate_name=proof-risk` row with `producing_invocation_uuid`, `canonical_output_path=${planning_dir}/risk/${wu_lower}-proof-risk.md`, `size`, `mtime`, `sha256`, `verdict_line`, and `verified_at`. This row is independent from `risk-assessor` because proof-risk evaluates proof-method evidence class, not uncovered-code priority.
- Rule: when no termination signal fires, any `MEDIUM` or `HIGH` report means revise the proposal and re-run all five.
...
- Rule: after the four PR-review gates complete and before writing `phase-8-join-manifest.json`, dispatch `~/ai/workflows/code-quality.md` in pipeline-callable mode against the actual PR diff. The dispatch prompt path is `${scratch_dir}/prompts/${wu_lower}-phase-8-code-quality.md`; log path is `${scratch_dir}/logs/${wu_lower}-phase-8-code-quality.log`; slug is `${wu_lower}-phase-8`; inputs include `diff_path=<PR-diff>`, `proposal_path=<proposal-cited-by-PR-footer>`, `proof_plan_excerpt=<excerpt>`, and `runtime_claim=<claim>`. The aggregate output is `${planning_dir}/code-quality/${wu_lower}-phase-8/aggregate-code-quality.md`, with dispatch manifest `${planning_dir}/code-quality/${wu_lower}-phase-8/dispatch-manifest.md` and child reports under `${planning_dir}/code-quality/${wu_lower}-phase-8/reports/`.
- Rule: the Phase 8 code-quality fanout must mark `validation-integrity-auditor` as `Required=true` because the actual PR diff has runtime-claim context under the Phase 3 `## Proof plan` rule. It must mark `proof-risk-auditor` as `Required=true` because it validates the proposal cited in the PR body's close-keyword footer against the shipped diff context.
- Rule: record a fifth Phase 8 join-manifest row with `gate_name=code-quality`, `producing_invocation_uuid`, `canonical_output_path=${planning_dir}/code-quality/${wu_lower}-phase-8/aggregate-code-quality.md`, `size`, `mtime`, `sha256`, `verdict_line`, and `verified_at`. This row is consumed by Process-tree audit #3 alongside the four PR-review rows.
