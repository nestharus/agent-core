...
- Policy: dispatch `apply-gate-set(caller_mode=implementation-phase-8)` after Phase 7 readiness and before draft PR movement.
...
- Phase 8 capture documentation: after `apply-gate-set(caller_mode=implementation-phase-8)` returns `PASS` with process-tree audit #3 evidence cleared, the orchestrator dispatches the `claude-opus` closure judge and records `actual_story_points` and `actual_capture_method` as an orchestrator-owned closure artifact before Phase 9. The enum is `closer-best-effort | wall-time-derived | unmeasured`, the point set is `1, 2, 3, 5, 8, 13, 21, 40, 100`, and wall-time-derived is reserved as enum-only with no requirements for timers, durations, start/stop timestamps, or trace-derived time calculations.
