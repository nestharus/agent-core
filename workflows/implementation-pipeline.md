...
- Rule: Recursive Step 6c consumes the Step 6b output index at `${scratch_dir}/phase6/step6b-output-index.md`, emitted tests, test paths, and test-group identifiers for the matching `level_id`. For ACR-247 side-channel runs, the orchestrator calls `~/ai/workflows/step6c-consumption-side-file.md` before Step 6c dispatch, and the expected-process manifest carries the side-file evidence for the scoped Step 6b output index and each implemented Step 6b output-index row. Child rows must preserve `<level_id>:<local_artifact_id>` scoping where the index uses string artifact identifiers.
...
- Rule: for docs/workflow WUs, regression-guard responsibility is deferred to the eval framework per ACR-175; Step 6b must not add markdown structural shape tests as behavior evidence.
