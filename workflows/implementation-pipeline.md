...
- Tests authored in **Phase 6b** and product code authored in **Phase 6c** **are** committed in the working repository and therefore land in the worktree. The test/code boundary is the planning-vs-product boundary. Exception: for `~/ai` markdown/operator/workflow/convention routing or anchor surfaces, structural-verification acceptance criteria route to WRITE-state eval specs under `evals/<slug>/eval.md`, not to pytest files, `tests/test_*.py`, or `tools/<wu>-verify/<anything>.py`.
...
- Rule: for `~/ai` markdown/operator/workflow/convention routing or anchor surfaces, "structural test", "regression-guard", "anchor verification", "shape test", "structural verifier", "structural-shape test", and similar doc-shape coverage language is not an uncovered behavior requiring characterization tests. Route it to eval-spec authoring per `~/ai/conventions/evals.md`; do not dispatch test-writer for `tools/<wu>-verify/<anything>.py`, `tests/test_*.py`, pytest imports, or pytest-shaped assertion code in `~/ai`.
...
- Rule: when the WU changes `~/ai` markdown/operator/workflow/convention routing or anchor surfaces and the requested verification is structural ("structural test", "regression-guard", "anchor verification", "shape test", "structural verifier", "structural-shape test", or similar), the test-intent track must name the item as an eval-spec intent with target path `evals/<slug>/eval.md` and lifecycle `WRITE`. It must not request test-writer, pytest, `tests/test_*.py`, or `tools/<wu>-verify/<anything>.py` output.
...
- Rule: `~/ai` markdown/operator/workflow/convention routing or anchor surfaces use the eval-spec replacement path for structural verification. If the proposal, contract, risk, or acceptance criteria say "structural test", "regression-guard", "anchor verification", "shape test", "structural verifier", "structural-shape test", or similar doc-shape language, Step 6b dispatches eval-spec authoring for `evals/<slug>/eval.md` in WRITE state. Step 6b must not emit `tools/<wu>-verify/<anything>.py`, `tests/test_*.py`, pytest imports, pytest fixtures, or pytest-shaped assertion code anywhere in `~/ai`.
...
- Output for `~/ai` structural-verification doc/anchor items: WRITE-state eval specs at `evals/<slug>/eval.md`, with the Step 6b output index mapping each structural intent to its eval spec path and lifecycle. No pytest/verifier file is a valid output for this surface class.
...
- Rule: Recursive Step 6c consumes the Step 6b output index at `${scratch_dir}/phase6/step6b-output-index.md`, emitted tests, test paths, and test-group identifiers for the matching `level_id`. Its captured log must contain relaxed-position `consumed:` rows for the scoped Step 6b output index and each implemented Step 6b output-index row before product-code changes. The rows may appear anywhere after runner-owned `OULIPOLY_INVOCATION` and `OULIPOLY_SESSION` envelope lines, and child rows must preserve `<level_id>:<local_artifact_id>` scoping where the index uses string artifact identifiers.
...
- Rule: Step 6c log output must contain relaxed-position `consumed:` rows proving which Step 6b test output paths and Step 6b output index paths it read before product-code changes. The rows may appear anywhere after runner-owned envelope lines; missing, malformed, contradicted, stale, or unmapped rows do not satisfy Phase 6 firstness evidence.
...
- Rule: for docs/workflow WUs, regression-guard responsibility is handled by the eval framework per ACR-175; Step 6b emits WRITE-state eval specs for structural doc/anchor verification intent and must not add markdown structural shape tests as behavior evidence.
...
- Rule: Process-tree review: after Step 6c completes and before Phase 7, run `process-tree-auditor` on the Phase 6 subtree. The expected process must prove separate Step 6b and Step 6c invocations, timing order, Step 6b output index presence, Step 6b output paths, relaxed-position Step 6c `consumed:` log rows for those outputs, and the derivation record or no-split / rejection subsection when a derivation trigger fires, plus `HaltRecord` or halt/no-split evidence when halt logic fires. The `consumed:` rows may appear anywhere after runner-owned envelope lines, but missing, malformed, mismatched, stale, or wrong-scope rows are blocking. When coupling examination changes or confirms the current component inventory, the expected process also proves the `CouplingDecision` record, its auditor evidence refs, and any rerun `integration_test_refs` are present before Phase 7. A `blocking` independence, output/artifact, or silent-success violation prevents CodeRabbit. Missing required evidence is `NEEDS_INPUT:<question_artifact>` when it can still be supplied, otherwise `blocking`; the affected subtree is `rerun or repaired` before downstream consumption.
