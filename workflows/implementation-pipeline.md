...
- Rule: when predecessor evidence supplies inherited prototype proof tests, the Phase 3 test-intent track must list each inherited prototype test path or node ID as a required test-intent item and carry forward the no-silent-drop / strictly stronger equivalent supersession rule from `~/ai/conventions/prototype-pending-tests.md` § `Carry-forward to implementation`.
- Rules: include a mandatory `## Proof plan` section with exactly these parse anchors:
  - `**Runtime claim**: <one or more sentences naming the runtime behavior asserted by the fix>`
  - `**Proof method**: <description of validation surface(s) that will exercise the runtime claim>`
  - `**Evidence-class match**: <explicit statement that the proof method's evidence class matches the runtime claim's required evidence class>`
  The proof plan is a proposal artifact, not a self-certification. Runtime-artifact-bound claims must name the runtime-artifact evidence class that will validate them; proxy evidence must be scoped to proxy-layer claims or paired with matching runtime-artifact proof.
...
- Artifacts represented by the gate-set: `${planning_dir}/risk/${wu_lower}-phase-3-estimate-writeback.json`, `risk/NN-audit.md`, `risk/NN-scope.md`, `risk/NN-shortcut.md`, `risk/NN-supported-surface.md`, `risk/NN-proof-risk.md`, Phase 4 code-quality outputs, `${planning_dir}/risk/phase-4-join-manifest.json`, expected-process evidence, process-tree audit #1, and optional bootstrap-exception ratification.
...
- `proof-risk`: does the proposal's `## Proof plan` map each runtime claim to a proof method whose evidence class exercises that claim. It remains a distinct row or an explicit ACR-285 inventory-resolution row with equivalent blocking semantics.
