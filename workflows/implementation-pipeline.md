...
Run the risk gate on the proposal artifact, the `problem map`, the supported-surface track, and the Phase 2.5 risk profile, not on hand-waved intent. The orchestrator composes one `apply-gate-set` prompt and dispatches `apply-gate-set(caller_mode=implementation-phase-4)` after Phase 3 proposal and estimate update, before Phase 5 hookpoint research.

- Artifacts represented by the gate-set: `risk/NN-audit.md`, `risk/NN-scope.md`, `risk/NN-shortcut.md`, `risk/NN-supported-surface.md`, `risk/NN-proof-risk.md`, Phase 4 code-quality outputs, `${planning_dir}/risk/phase-4-join-manifest.json`, expected-process evidence, process-tree audit #1, and optional bootstrap-exception ratification.
...
- Phase 8 capture documentation: after `apply-gate-set(caller_mode=implementation-phase-8)` returns `PASS` with process-tree audit #3 evidence cleared, the orchestrator dispatches the `gpt-high` closure judge and records `actual_story_points` and `actual_capture_method` as an orchestrator-owned closure artifact before Phase 9. The enum is `closer-best-effort | wall-time-derived | unmeasured`, the point set is `1, 2, 3, 5, 8, 13, 21, 40, 100`, and wall-time-derived is reserved as enum-only with no requirements for timers, durations, start/stop timestamps, or trace-derived time calculations.
