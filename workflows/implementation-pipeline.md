...
Immediately after the audit fanout returns and before Phase 2.5 or Phase 3 consumes the aggregate, completion follows `~/ai/workflows/agents-cli.md` § `Long-running agents`: Bash task notifications are primary, then terminal sentinel markers plus durable bundle artifacts are verified before consumption, with bounded failure on missing completion evidence. The orchestrator dispatches `process-tree-auditor` on the audit fanout. The later Phase 4 process-tree audit still runs and its expected process includes the entry-mode audit subtree when the proposal depended on audit evidence.
...
- That workflow covers five pre-synthesis gates: test-audit review, multi-concern PR review, justification PR review, supported-surface verification, and commit hygiene.
- Rule: do not duplicate the full procedure here.
- Rule: Gate fanout completion follows `~/ai/workflows/agents-cli.md` § `Long-running agents`: Bash task notifications are primary, terminal sentinels and canonical artifacts are verified before report consumption or join-manifest write, fallback reconciliation is bounded, and cleanup-trap evidence is required where the orchestrator owns backgrounding.
