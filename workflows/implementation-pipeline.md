...
Evidence-class convention: `~/ai/conventions/evidence-class.md`
...
- **Evidence-class continuity.** For claims about runtime behavior, a green test or eval is not automatically runtime evidence. Runtime-scoped surfaces carry an evidence-class entry per `~/ai/conventions/evidence-class.md` so Phase 3, Phase 4, Phase 6, and Phase 8 can compare the evidence class required by the claim against the class actually supplied.
...
- Artifacts (all required): `research/NN-problem-map.md`, `research/NN-coverage-inventory.md`, `research/NN-lifecycle-map.md`, `research/NN-entrypoints.md`, `research/NN-duplicates.md`, `research/NN-cross-language-trace.md`, `risk/NN-risk-profile.md`. When the current-state map identifies a runtime-scoped claim, a changed validation surface, or an unclear runtime-vs-test evidence boundary, Phase 2.5 also authors `${planning_dir}/evidence-class.md` per `~/ai/conventions/evidence-class.md`.
...
- Rule: the evidence-class ledger is per-surface and records `required_evidence_class`, current `supplied_evidence_class`, validation surface, runtime surface, producer phase, expected consumers, and status. The risk profile may cite this ledger as risk evidence, but it does not replace it.
...
- Inputs: approved `problem map`, coverage inventory, lifecycle map, entrypoints, duplicates, cross-language trace, halt records and halt residual evidence when present, **risk profile**, and `evidence-class.md` when present from Phase 2.5; any research from Phases 1-2.
...
- Rule: validation-surface integrity. When the WU touches tests, fixtures, mocks, baselines, eval specs, proof plans, skips/xfails, CI setup, dependency setup, or any validation gate, the proposal must name the previous validation meaning, whether the proposal changes that meaning, the corresponding product/runtime-artifact fix, and the replacement validation against the supported runtime artifact or path. For runtime surfaces outside the test harness, the test-intent track must distinguish test-environment evidence from runtime-artifact evidence.
- Rule: when any test-intent item is used as proof for a runtime-scoped claim, the proposal must cite or update the corresponding `evidence-class.md` entry, including `required_evidence_class` and the proposed `supplied_evidence_class`. A proposal that plans only `validation-proxy`, `static-or-documentary`, or `unknown` evidence for a `runtime-path` required claim must add runtime-path proof or explicitly hand Phase 4 a supported-surface/value-collapse risk to judge.
...
- `audit risk`: presence, contracts, migrations, test-intent track, fixture source, residual-risk artifact, evidence-class entries for runtime-scoped claims, and other checklist obligations
...
- validation-surface integrity risk: the audit/shortcut/supported-surface gates must treat weakening the signal that exposes the risk as defeating the underlying purpose unless the proposal includes a legitimate validation correction plus a corresponding product/runtime-artifact fix and replacement runtime validation. A proposal whose proof path relies only on a test-environment substitute for a runtime/deployment claim cannot return `LOW`.
...
- Rule: Phase 4 code-quality gate. After the four proposal-risk gates return current `LOW` and before the Phase 4 join manifest, the orchestrator runs `~/ai/workflows/code-quality.md` in pipeline-callable mode over current WU evidence (`proposals/NN-*.md`, `research/NN-problem-map.md`, `risk/NN-risk-profile.md`, touched-surfaces inventory, and any current diff/changed-files/changed-functions evidence). That evidence identifies touched files/components; the blocking target is the whole touched file/component, not only diff hunks or changed functions. Per `~/ai/conventions/code-quality.md` § `Disposition policy`: Only LOW passes the Phase 4 code-quality gate. `MEDIUM` and `HIGH` block advance, trigger remediation/revise, and require a rerun from current evidence. Neither severity is ever a residual, a `NEEDS_INPUT` user-disposition choice, or a stable allow-advance state. After two consecutive non-converging remediation rounds, the WU decomposes autonomously per `~/ai/conventions/code-quality.md` § `Oscillation signals WU-too-large`: decompose is not `NEEDS_INPUT`-mediated, the orchestrator records the oscillation evidence in `${planning_dir}/audit-history.md`, files the smaller sub-tickets through the project's ticket operator, and returns `DECOMPOSED` to the root caller instead of attempting a third remediation pass.
...
- Rule: Phase 6 consumes the approved `research/NN-problem-map.md`, approved `proposals/NN-*.md` including its test-intent track and assumption register, `risk/NN-supported-surface.md`, `evidence-class.md` when present, and `research/NN-hookpoints.md`.
- Rule: Step 6a preserves required evidence class in the contract. Step 6b maps each test, eval spec, runtime command, or report to the supplied evidence class it can provide. Step 6c is not complete for a runtime-scoped claim until the output index or report evidence supplies the required class or records a blocking mismatch.
...
- Rule: the contract must preserve every change risk or verification risk, selected test level, fixture source, assumption-register link, expected observable signal, and evidence-class requirement from the approved proposal test-intent track.
...
- Rule: Phase 6 prototype risk review runs `risk-assessor` after Step 6c produces a passing level prototype and level behavior tests pass, and writes `${planning_dir}/risk/${wu_lower}-prototype-risk.md` with verdict vocabulary `LOW`, `MEDIUM`, and `HIGH` before downstream derivation, `PrototypeSwapRecord`, or before Phase 7 consumes the prototype. The dispatch, artifact check, verdict check, refusal behavior, audit-history append, and allow-advance condition live in the Phase 6 prototype risk review sub-step in `agents/implementation-pipeline-orchestrator.md`. This rule reuses the existing `risk-assessor` operator and does NOT redefine its output contract, and it is distinct from `~/ai/workflows/build-prototype.md` exploratory prototype `dossier/risk-profile.md` lifecycle.
...
- Rule: Step 6c must not make a failing or risk-bearing signal pass by weakening tests, fixtures, mocks, baselines, eval specs, skips/xfails, dependency setup, CI setup, or harness behavior. If the required fix is a runtime/deployment fix, the product/runtime artifact must change and the Step 6b output index must retain the original validation meaning or record a strictly stronger replacement validation. A validation-only green result is not complete Step 6c work.
...
- Rule: Process-tree review: after Step 6c completes and before Phase 7, run `process-tree-auditor` on the Phase 6 subtree. The expected process must prove separate Step 6b and Step 6c invocations, timing order, Step 6b output index presence, Step 6b output paths, ACR-247 side-channel consumption evidence carried by the expected-process manifest for those outputs, and the derivation record or no-split / rejection subsection when a derivation trigger fires, plus `HaltRecord` or halt/no-split evidence when halt logic fires. The side-channel carrier comes from the sibling projection workflow before Step 6c dispatch; missing, malformed, mismatched, stale, late, or wrong-scope side-file evidence is blocking. When coupling examination changes or confirms the current component inventory, the expected process also proves the `CouplingDecision` record, its auditor evidence refs, and any rerun `integration_test_refs` are present before Phase 7. A `blocking` independence, output/artifact, or silent-success violation prevents CodeRabbit. Missing required evidence is `NEEDS_INPUT:<question_artifact>` when it can still be supplied, otherwise `blocking`; the affected subtree is `rerun or repaired` before downstream consumption.
...
- Rule: Phase 8 Test Audit is the final validation-surface integrity check on the actual diff. It must compare changed validation surfaces against changed product/runtime-artifact surfaces and block when a test, fixture, mock, baseline, eval spec, skip/xfail, dependency setup, CI setup, or harness edit weakens runtime meaning without a corresponding runtime-artifact fix and replacement runtime validation. The same gate consumes `${planning_dir}/evidence-class.md` when present and blocks a runtime-scoped claim whose supplied evidence class does not satisfy the required evidence class.
