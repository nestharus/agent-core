...
workflow_dispatch_contract:
  ...
  expectations:
    - "runs research, proposal, shared apply-gate-set risk/review boundaries, hookpoint, implementation, readiness, PR, and audit phases"
    ...
    - "requires every Phase 3 proposal to expose Behavior claim, Experiment command or action, Expected observation, and Claim-experiment fit under one exact ## Verification plan section"
    ...
    - "Phase 0, Phase 7, and Phase 9 invoke exact wu-session-migration operations against one declared live runtime root: direct WUs use the top-level project planning tree and feature direct/refactoring children use their normalized F/routes root; the shared executable owns lock, journal, held-parent, rollback, and recovery mechanics while sessions.index.json remains historical source inventory"
  ...
  outputs:
    ...
    - "planning_dir/../sessions.active-wake.json canonical owner row after exact Phase 7 PR acquisition"
...
adapter_declarations:
  - component: workflows/implementation-pipeline.md
    role: adapter
    Translates:
      - implementation-pipeline-lifecycle-surface
      - phase-4-6-8-apply-gate-set-caller-surface
      - currentness-policy-citation-surface
```

The three translated surfaces are the complete adapter declaration. The new currentness-policy-citation-surface subordinates the `~/ai/conventions/apply-gate-set-currentness.md` § `Currentness key schema`, § `Row-level re-verification`, and § `Full re-dispatch` citations to one translated contract.
...
- Phase 7 and Phase 9 maintain the one `sessions.active-wake.json` owned by exact live runtime root `${planning_dir}/..` with the session manifest under the shared exclusive cutover lock and interruption-recoverable journal protocol; direct WUs use the top-level project planning tree and feature direct/refactoring children use their normalized `F/routes`; `sessions.index.json` remains historical source inventory
- each Phase 8 rerun allocates a new exact-identity PR-review run and keeps every runner log distinct from its canonical result while preserving prior run lineage
- a feature-routed draft result exposes `implementation-pipeline-result-v1` with `base_branch` equal to the dispatched feature branch, `base_ref` equal to its fetched `refs/remotes/origin` identity, reviewed/current provider `base_ref_name` equal to that exact branch even when OIDs coincide, and current Phase 4/6/8 proof rows for the parent's common `feature-route-attempt-proof-v1` gate; direct merge authorization remains a separate parent composition with ticket/currentness/acceptance evidence
...
The orchestrator comments back to the ticket on Phase 9 (PR URL) and on Final (audit-history close + decision tail). Routine Work Unit status transitions are manager-owned: the manager moves Todo to In Progress at dispatch and moves In Progress back to Todo after permanent dispatch failure with no PR. For Linear-backed WUs, the manager may rely on GitHub close-keyword automation for In Progress to Done after merge. If Linear close-keyword automation is unavailable, misconfigured, or requires manual verification, the manager performs the Done transition explicitly through the selected ticket operator. Non-Linear backends follow their existing project transition policy; do not assume GitHub close-keyword automation. The implementation-pipeline orchestrator itself does not transition status.
...
Staleness follows the audit workflow currentness predicates, not mtime cleanup heuristics. Apply-gate-set row currentness is separate and lives in `~/ai/conventions/apply-gate-set-currentness.md`. `review_staleness_policy=exact-match` requires the imported target identity and current target identity to match. `required` reruns audit before proposal work and treats the prior bundle as context. `allow-with-drift-report` requires a drift report mapping changed files or sections; stale non-LOW findings may seed still-existing target items, but stale LOW/no-drift reports are context only for changed targets and require current re-audit before closure.
...
- Exception: for `~/ai` markdown/operator/workflow/convention/routing/anchor structural-verification routes, Step 6b commits WRITE-state eval specs at `evals/<slug>/eval.md` instead of test files in project test roots. The binding output prohibition is the one in `~/ai/conventions/evals.md`: no `tools/<wu>-verify/<anything>.py`, no `tests/test_*.py`, no pytest imports or fixtures, and no pytest-shaped assertion code.
...
`layer_level_id`, when used by recursive Phase 6 records, is the current layer's composed identity over `level_id`, not an independent scope key or axis; string artifact identifiers still use `<level_id>:<local_artifact_id>`.
...
- Rule: Phase 0 does not edit product source. When Phase 0 must create a reproduction signal, it adds tests only; accepting existing failing evidence adds no source edits.
- Rule: for E2E frontend element timeouts, follow the page-state-first branch in `agents/incident-investigator.md` before treating the failure as load-time.
- Gate: model. The orchestrator advances autonomously to Phase 2.5 once each named root cause has a newly authored reproduction test, an existing failing signal accepted as reproduction evidence, or a documented `Hypothesis (unreproduced)` with the specific evidence that would confirm or refute it. Surface a NEEDS_INPUT new-value-question to the root only if the reproduced or accepted cause invalidates the framing of the original request.
...
Phase 1 and the Phase 2.5 problem map consume inherited estimate metadata from `${scratch_dir}/ticket.md`, rendered by Phase 0 from the configured ticket system. The frontmatter shape is `story_point_estimate`, `estimate_source`, `estimate_rationale`, and `estimate_field`; `estimate_field` is Linear `estimate` or the Jira field resolved from the selected operator contract. When `estimate_source` is `backstop-spike` or `missing` and the user has not dispositioned that cold start, the orchestrator surfaces the root-bound `NEEDS_INPUT` new-value question at `${scratch_dir}/questions/q-<uuidv4>.question.json` with `single_choice` options `Run a small prototype first`, `Proceed without a baseline estimate`, and `Terminate WU`, then returns `NEEDS_INPUT:<absolute_artifact_path>`. After an accepted answer, the answer file remains durable and hash-verified while the orchestrator invokes `wu-session-migration cold-start-disposition-bind` with a null row identity; no caller writes the manifest or index directly. Caller-owned readback must prove that only `cold_start_disposition_ref` changed and that the complete active-index bytes and identity did not change before the separate terrain/risk/defer gate can run. A failed readback blocks without replay. Phase 1 outputs, the problem map, and sub-step research propagate the inherited estimate values verbatim so Phase 3 can compare against the same baseline.
...
- Role: build the approved understanding of the touched surface as it exists today, **scored** for risk per `~/ai/conventions/risk-profile.md`. Inputs are the planned change surface: files, symbols, endpoints, commands, jobs, or workflows expected to change. When prior recursion artifacts exist, `HaltRecord` artifacts (including their artifact paths), `HaltRecord.residual_risk_refs` entries, and halt residual evidence are first-class current-state inputs for these same artifacts.
...
- Rule: when the uncovered gap is a `~/ai` markdown/operator/workflow/convention/routing/anchor structural-verification gap, suppress characterization pytest/verifier authoring. The characterization need routes forward as WRITE eval-spec intent under `~/ai/conventions/evals.md` rather than to `tests/test_*.py`, pytest imports or fixtures, pytest-shaped assertion code, or `tools/<wu>-verify/<anything>.py`.
...
- Rule: scoring consumes prior halt records, `HaltRecord.component_candidates_considered`, `HaltRecord.residual_risk_refs`, and `auditor_verdict_conflict_residual` entries when they affect any risk axis.
...
- Rule: keep the map bounded to touched-file/component ownership plus adjacent supported paths. Do not expand into a whole-system inventory, but do include pre-existing code-quality debt inside files/components the WU plans to touch so Phase 2.5 can estimate, own, or decompose it.
...
- The duplicates inventory (sub-step 2.5.4) finds a sprawling parallel-systems landscape outside touched-file/component ownership, or inside touched ownership but too large for one coherent WU, that the WU cannot navigate without architectural decisions or decomposition.
...
<!-- INTENTIONAL: "defer to prototype" is the user-visible Phase 2.5 disposition, not an unwired future-ticket stub; the implementation-pipeline orchestrator wires the dispatch, ticket marking, cross-link comment, and halt in its Phase 2.5 human-gate branch. -->
When two or more of these fire, the Phase 2.5 human gate surfaces options: `proceed in exhaustive mode (the WU is just big)`, `defer to prototype (framing is too unclear; build a prototype to clarify, then re-ticket from its dossier)`, `terminate WU (the work is wrong-shaped; abandon)`. Choosing **defer to prototype** dispatches `~/ai/agents/prototype-orchestrator.md` per `~/ai/workflows/build-prototype.md` with the WU's question as the prototype's question and the WU's `${ticket_id}` (`jira_issue_key` or `linear_issue_key`, per project) as `defer_source`. Before the implementation orchestrator halts the deferred WU, it marks the original ticket as deferred with backend-specific evidence: the Linear path applies `deferred-to-prototype` through `linear-operator task=apply-labels create_missing=true replace=false`; the Jira path attempts `jira-operator task=transition target_status=Blocked`, and if `Blocked` is unavailable the backend fallback is a comment-only deferred marker. Sprint/cycle removal is attempted only where a declared backend operation exists; with the current Linear/Jira operator surfaces it is unsupported and recorded as `fallback:operationally-manual` in the immediate comment and execution evidence. The immediate comment cross-links the prototype identity or tracker key, the prototype dossier path or pending dossier path, the deferred marker, and the sprint/cycle removal fallback. When the prototype later publishes a prototype-test PR, spawned implementation tickets must inherit the prototype-test PR URL, branch, test paths/node IDs, marker reason, ticket mapping, and acceptance criterion from `~/ai/conventions/prototype-pending-tests.md` § `Carry-Forward To Implementation`: remove the `prototype-pending:` markers, make the inherited pending production behavior tests pass, and preserve original assertions unless a traceable strictly stronger equivalent supersession is recorded. The implementation orchestrator halts the deferred WU; the prototype's dossier later spawns new tickets that re-enter the implementation pipeline with clearer scope.

worked example: a WU defers at Phase 2.5, so the original Linear ticket receives the `deferred-to-prototype` label, or the original Jira ticket moves to `Blocked` with a comment-only backend fallback when that transition is unavailable. The cross-link comment names the prototype identity or tracker, records that sprint/cycle removal is unsupported through current operators unless a backend adds the operation, and points to the pending dossier. The prototype dossier ships with spawned tickets and an approved original-ticket disposition. If the dossier says `close-as-superseded`, prototype P4 files the spawned tickets first, then closes the original as superseded where the backend supports that status and writes backend-specific links/comments: Jira uses supported issue links such as `Cloners`, while Linear uses parent/sub-issue where possible or durable comments. If backend support is missing, the fallback is recorded rather than silently claiming the operation happened.
...
- Inputs: approved `problem map`, coverage inventory, lifecycle map, entrypoints, duplicates, cross-language trace, halt records and halt residual evidence when present, and **risk profile** from Phase 2.5; any research from Phases 1-2.
- Role: propose design, scope, architecture, tradeoffs, `Forbidden behaviors`, a test-intent track, and the specific current-state risk the change reduces. `Forbidden behaviors` is behavior-forbidding only; it must not state work-narrowing exclusions that bypass touched-file/component ownership. **Do not** implement.
...
- Rule: when approved Phase 2.5 inputs include `HaltRecord` evidence or halt residuals, the proposal must state whether it preserves, revises, invalidates, or pays down those prior residuals, including `auditor_verdict_conflict_residual`.
...
- Rule: when the test-intent item is `~/ai` markdown/operator/workflow/convention/routing/anchor structural-verification, the test-intent track encodes lifecycle `WRITE` eval-spec authoring at `evals/<slug>/eval.md`, not pytest test files or one-off verifier scripts.
- Rule: when predecessor evidence supplies pending production behavior tests, the Phase 3 test-intent track must list each inherited prototype test path or node ID as a required test-intent item and carry forward the no-silent-drop / strictly stronger equivalent supersession rule from `~/ai/conventions/prototype-pending-tests.md` § `Carry-forward to implementation`.
- Rules: follow `conventions/behavioral-proof.md` and include one mandatory `## Verification plan` section with exactly these parse anchors:
  - `**Behavior claim**: <behavior and scope to be established>`
  - `**Experiment command or action**: <concrete executable test, deterministic inspection, build/run, or application-use action>`
  - `**Expected observation**: <predeclared result that would support the claim at the stated scope>`
  - `**Claim-experiment fit**: <why the experiment directly exercises the claim, or the honestly narrowed proxy scope>`
  The verification plan is a proposal artifact, not self-certification or an observed result. Runtime-artifact-bound claims must name build and execution of the target artifact; proxy evidence must be scoped to proxy-layer claims or paired with the matching runtime experiment.
...
The orchestrator validates every displayed field and every nested `estimate_delta_flag` member. `inherited_story_point_estimate` must be present even when null, the nested inherited/refined values must equal their sibling fields, and null inherited requires `over_2x: unknown`.

Phase 0 resolves estimate mutation policy from the same authoritative optimized ticket-operator contract selected by wrapper precedence. `estimate_mutation_enabled=false` on that selected contract selects `no_write_policy_disabled`; a base-contract value is backend-global, while a current project wrapper may provide project-local policy through normal precedence. `true` selects the mutation path. When policy is absent, the legacy mutation path is allowed only when the resolved contract advertises both `task=update-estimate` and the matching backend mutation side effect. Caller/session overrides, malformed/conflicting policy, unbound policy, or absent capability evidence block before Phase 3 ticket action.

After Phase 3 artifact verification and before Phase 4 prompt composition, the orchestrator writes `${planning_dir}/risk/${wu_lower}-phase-3-estimate-writeback.json` using schema `phase-3-estimate-writeback-v1`. The artifact binds ticket/backend, resolved operator and optimized-contract paths/hashes, policy source and disposition, estimate field, Phase 0 ticket snapshot path/hash/producer, Phase 3 proposal path/hash/producer, inherited/refined estimates, source, rationale, complete `estimate_delta_flag`, null-baseline disposition reference, dispatch expected/executed booleans, mutation evidence when applicable, and currentness identity. It is deterministic sorted-key JSON with no timestamp and must not claim a live ticket change on the no-write path.

For an enabled or legacy-capable contract, the orchestrator dispatches `${ticket_operator}` once with `task=update-estimate`, requires the operator's durable rationale and verified success contract, and records `disposition=write_verified`. Any backend, network, authorization, validation, or operator failure blocks and cannot become no-write evidence. For authoritative selected-contract `false`, it does not compose, dispatch, or simulate `update-estimate` and records `disposition=no_write_policy_disabled`. The Phase 4 scope-risk prompt always receives the same complete `estimate_delta_flag`; inherited null requires `over_2x: unknown`, never `false`.

After the estimate artifact and every identity it names are durably verified, the orchestrator invokes `wu-session-migration phase3-bind` with a null row identity. This operation preserves the already-bound cold-start reference, changes only the estimate reference/hash, and appends one canonical Phase 3 completion history object. The active index and every artifact are source-identity-bound read-only guards, not write targets. When authenticated `phase0-reresolve` producer identities differ from the original branch-out blobs, the canonical closed re-resolution readback is an additional required read-only guard and is machine-validated against the unchanged pre-Phase-3 manifest; it is never synthesized or replayed. Caller-owned `wu-session-pre-pr-write-readback-v1` evidence must prove the exact changed keys, exact history prefix/append, current artifact hashes, unchanged active-index bytes/device/inode/mode/rows, no synthesized row, and no retained journal. The orchestrator must run `wu-session-migration validate-pre-pr-readback --readback <path>` over that closed evidence; Phase 4 cannot be composed until validation succeeds, and a literal `verdict=PASS` is insufficient. Direct writes, a combined cold-start/Phase-3 projection, fallback to another operation, and semantic replay are forbidden.

Existing sessions do not silently adopt this disposition contract. Pre-Phase-3 sessions re-enter Phase 0 to resolve and hash current operator/contract policy and reread the ticket, then use the exact `wu-session-migration phase0-reresolve` transaction and closed readback before continuing. A proposal without disposition cannot enter Phase 4. A previously verified mutation may avoid a duplicate write only after exact current issue/field/value readback under matching ticket snapshot, proposal, operator/contract, policy, and field identities; otherwise affected Phase 4 and later joins are invalidated and audit/session migration lineage is required. Sessions already past Phase 4 rerun Phase 4 so the new writeback row is present; sessions blocked before Phase 2.5 or Phase 3 restart from Phase 0 and cannot reuse pre-disposition join manifests.

## Phase 4 - Risk Gates (required; apply-gate-set)

Run the risk gate on the proposal artifact, the `problem map`, the supported-surface track, the Phase 2.5 risk profile, and the Phase 3 estimate-writeback disposition, not on hand-waved intent. The orchestrator composes one `apply-gate-set` prompt and dispatches `apply-gate-set(caller_mode=implementation-phase-4)` after Phase 3 proposal and estimate disposition, before Phase 5 hookpoint research.

- Artifacts represented by the gate-set: `${planning_dir}/risk/${wu_lower}-phase-3-estimate-writeback.json`, `risk/NN-audit.md`, `risk/NN-scope.md`, `risk/NN-shortcut.md`, `risk/NN-supported-surface.md`, `risk/NN-verification-plan-review.md`, Phase 4 code-quality outputs, `${planning_dir}/risk/phase-4-join-manifest.json`, expected-process evidence, process-tree audit #1, and optional bootstrap-exception ratification.
- `phase-3-estimate-writeback`: verifies `write_verified` with mutation/readback evidence or `no_write_policy_disabled` with contract-bound wrapper policy and no update-estimate child. The no-write expected-process entry is forbidden-if-present; any unexpected estimate-write child blocks. Missing, malformed, stale, caller-overridden, hash-mismatched, or task-failure-derived disposition evidence blocks.
...
- `verification-plan-review`: does the proposal's `## Verification plan` map each behavior claim to an executable experiment and predeclared expected observation that directly exercise that claim. It remains a distinct row or an explicit ACR-285 inventory-resolution row with equivalent blocking semantics. `LOW` authorizes plan execution only; it is not behavioral proof.
- `code-quality`: Phase 4 pipeline-callable code-quality over current WU evidence remains part of the Phase 4 gate-set. Only `LOW` passes unless a valid bootstrap-exception row ratifies a non-LOW aggregate under `~/ai/conventions/code-quality.md` § `Bootstrap exception`.
- Rule: `apply-gate-set(caller_mode=implementation-phase-4)` writes or verifies the canonical Phase 4 join manifest at `${planning_dir}/risk/phase-4-join-manifest.json`, projects expected-process evidence, and requires process-tree audit #1 evidence. Rows carry at least `canonical_output_path`, `size`, `mtime`, `sha256`, `verdict_line`, and `verified_at`, plus the operator schema's caller mode, raw/normalized verdict, applicability, currentness, ratification, inventory-resolution, and blocking-status fields. Currentness keys and row-level reuse follow `~/ai/conventions/apply-gate-set-currentness.md` § `Currentness key schema` and § `Row-level re-verification`.
- Rule: scope-risk consumes the artifact's inherited/refined values, rationale, null-baseline disposition, and full `estimate_delta_flag` plus the writeback-disposition reference. It must preserve `over_2x: unknown` for a null inherited value; missing absolute-scope evidence or normalization of `unknown` to `false` blocks.
- Rule: the bootstrap-exception parser remains orchestrator-owned. The orchestrator parses the proposal `## Bootstrap exception declaration`, parses the matching DECISIONS heading and convention citation, and passes ratification evidence into the gate-set. The operator writes the row; it does not become the authority for the proposer/orchestrator four-condition declaration.
- Rule: in **lean mode** (per `~/ai/conventions/risk-profile.md` § Pipeline mode) the supported-surface gate may pass at LOW with a single-paragraph supported-surface track and the audit gate may pass at LOW without per-AC residual entries. In **exhaustive mode** the gates require full evidence.
...
- Rule: when no termination signal fires, any current, well-formed semantic `MEDIUM` or `HIGH` proposal-risk report means revise the proposal and rerun the Phase 4 gate-set from clean state. Do not keep old `LOW` reports after a substantive proposal revision.
- Rule: preserve ACR-280 strategy selection. Supported-surface non-LOW records `STRATEGY_PHASE4_SUPPORTED_SURFACE_INWU` or `STRATEGY_PHASE4_SUPPORTED_SURFACE_FOLLOWUP` when those classifications apply; scope decomposition records `STRATEGY_PHASE4_SCOPE_DECOMPOSED`; code-quality non-LOW that is not ratified by bootstrap exception records `STRATEGY_PHASE4_CODE_QUALITY_INWU`, `STRATEGY_PHASE4_CODE_QUALITY_HELPER_EXTRACTION`, `STRATEGY_PHASE4_CODE_QUALITY_MOVE_AND_IMPORT`, in-place file-decomposition, or follow-up-ticket decomposition according to the current finding shape.
...
- Rule: if a proposer claims it "re-ran the risks itself," ignore that claim and rerun or reverify through the actual Phase 4 gate-set.
...
When Step 6c's post-prototype derivation accepts internal components, current-layer integration tests for interactions between those component pairs are required as part of the same derivation evidence.
...
- Rule: Step 6b tests follow `~/ai/conventions/testing.md`; that canonical rule governs test authorship for setup placement, setup swapping, and cross-module setup.
- Rule: for `~/ai` markdown/operator/workflow/convention/routing/anchor structural-verification routes, Step 6b authors a WRITE-state eval spec instead of test files. The output is `evals/<slug>/eval.md` plus `${scratch_dir}/phase6/step6b-output-index.md`; `~/ai/workflows/build-prototype.md` may be referenced only as a prototype boundary when applicable, not as a reason to turn this route into prototype or pytest work.
- Rule: the structural-verification eval-spec route inherits the binding `~/ai/conventions/evals.md` prohibition: no `tools/<wu>-verify/<anything>.py`, no `tests/test_*.py`, no pytest imports or fixtures, and no pytest-shaped assertion code.
...
- Output: for structural-verification eval-spec routes, WRITE-state eval specs at `evals/<slug>/eval.md`, with lifecycle and finding contract fields required by `~/ai/conventions/evals.md`.
- Output: `${scratch_dir}/phase6/step6b-output-index.md`, listing every proposal test-intent item, named risk, selected level, source, emitted test file, test group, or eval-spec artifact path, residual entry, documented non-applicability reason, and listing implementation-discovered procedural obligations with their emitted procedural tests or residual entries.
- Output-index fields: approved proposal path, contract path, approved `problem map` path, `risk/NN-supported-surface.md` path, hookpoint research path, Step 6b prompt path, Step 6b log path, `level_id` when recursive Phase 6 work emits level-scoped artifacts, each test-intent item, named risk, selected level, proposal or assumption-register source, emitted test file path, eval-spec path, and test/eval/test-group identifier, residual entry path when the item maps to `risk/NN-test-residuals.md`, documented non-applicability reason when no test or eval spec is emitted, declared fixture source or fixture application point, procedural obligation, Step 6c evidence that discovered the obligation, emitted procedural test file path and test or test-group identifier when a procedural test was authored, or procedural residual entry path when no procedural test is emitted; residual class when no procedural test is emitted. The path `${scratch_dir}/phase6/step6b-output-index.md` remains the Step 6b output index path; recursive rows and artifact identifiers carry `level_id` and use `<level_id>:<local_artifact_id>` when a string identifier is needed.
- Rule: when predecessor evidence supplies inherited prototype `test_paths_or_node_ids`, the Step 6b output index must map every inherited prototype entry to a Step 6b-authored production-tree test, an existing passing production-tree test, or a recorded strictly stronger equivalent supersession entry in the manifest, spawned ticket payload, or Step 6b output index. A missing mapping is not non-applicability.
- Output-index fields for accepted internal components: integration-test intent at selected level `particular-integration`, the applicable `layer_level_id` / `level_id` scope, the `LevelComponentSet` artifact path, and later `integration_test_refs` when Step 6c completes concrete pair coverage after accepted internal components and interactions are known.
- Rule: When Step 6c records a `CouplingDecision`, the Step 6b output index also lists the `coupling_decision_ref`, affected `level_id`, source `LevelComponentSet` or candidate inventory reference, and any rerun `integration_test_refs` that prove the emitted tests still match the current component inventory.
- Output: `risk/NN-test-residuals.md` when any named risk remains unverified.
- Rule: Phase 6 prototype risk review is represented inside `apply-gate-set(caller_mode=implementation-phase-6)` after Step 6c produces a passing level prototype and level behavior tests pass. The required row preserves `${planning_dir}/risk/${wu_lower}-prototype-risk.md`, verdict vocabulary `LOW`, `MEDIUM`, and `HIGH`, and the existing allow/block semantics before downstream derivation, `PrototypeSwapRecord`, or Phase 7 readiness consumes the prototype. This row remains distinct from `~/ai/workflows/build-prototype.md` exploratory prototype `dossier/risk-profile.md` lifecycle.
- Rule: Phase 6 prototype-risk verdict semantics. Only `LOW` advances Phase 6 prototype risk to downstream derivation, `PrototypeSwapRecord` production or consumption, or Phase 7 advance. `MEDIUM` and `HIGH` model verdicts block advance, trigger remediation/revise from current evidence, and are never accepted as a residual, a `NEEDS_INPUT` user-disposition choice, or a stable allow-advance state. After two consecutive non-converging rounds, the WU decomposes autonomously per `~/ai/conventions/code-quality.md` § `Oscillation signals WU-too-large`: decompose is not `NEEDS_INPUT`-mediated, the orchestrator records the oscillation evidence in `${planning_dir}/audit-history.md`, files the smaller sub-tickets through the project's ticket operator, and returns `DECOMPOSED` to the root caller instead of attempting a third remediation pass.
- Rule: Phase 6 test-contracts alignment review consumes the Step 6a contract, Step 6b tests or structural-verification eval specs, and `${scratch_dir}/phase6/step6b-output-index.md`; checks them against the approved proposal test-intent track before Step 6c; and writes `${planning_dir}/alignment/${wu_lower}-tests-contracts.md` with verdict vocabulary `ALIGNED`, `MISALIGNED`, and `NEEDS_REVISION`. The workflow doc is the source of truth for this model-gate review's inputs, artifact path, and verdict vocabulary; the dispatch, artifact check, verdict check, refusal behavior, audit-history append, and allow-advance condition live in the Phase 6 test-contracts alignment review sub-step in `agents/implementation-pipeline-orchestrator.md`.

### Step 6c - Write code

- Inputs: contracts, `${scratch_dir}/phase6/step6b-output-index.md`, and the tests or structural-verification eval specs from Step 6b.
...
- Rule: Recursive Step 6c consumes the Step 6b output index at `${scratch_dir}/phase6/step6b-output-index.md`, emitted tests or structural-verification eval specs, test/eval paths, and test/eval-group identifiers for the matching `level_id`. For ACR-247 side-channel runs, the orchestrator calls `~/ai/workflows/step6c-consumption-side-file.md` before Step 6c dispatch, and the expected-process manifest carries the side-file evidence for the scoped Step 6b output index and each implemented Step 6b output-index row. Child rows must preserve `<level_id>:<local_artifact_id>` scoping where the index uses string artifact identifiers.
...
- Rule: Step 6c consumption evidence for ACR-247 side-channel runs is the manifest-declared side-file projected before dispatch by `~/ai/workflows/step6c-consumption-side-file.md`. The Step 6c prompt and log remain companion topology evidence, but model-authored `consumed:` rows, read-confirmation rows, or equivalent attestations are not load-bearing firstness evidence. Missing, malformed, contradicted, stale, unmapped, late, or wrong-scope side-file evidence does not satisfy Phase 6 firstness evidence.
- Rule: an implementation that inherited pending production behavior tests is incomplete while those tests are missing from the diff or output index, still marked `prototype-pending:`, unrun, failing, or replaced without a recorded strictly stronger equivalent supersession entry in the manifest, spawned-ticket payload, or Step 6b output index.
- Rule: per-component code-quality auditor fanout is represented inside `apply-gate-set(caller_mode=implementation-phase-6)` for each individual component after that component's Step 6c implementation and scoped tests pass, using `component_slug` to preserve `${planning_dir}/code-quality/${wu_lower}-${component_slug}/dispatch-manifest.md`, `${planning_dir}/code-quality/${wu_lower}-${component_slug}/reports/cohesion-auditor.md`, `${planning_dir}/code-quality/${wu_lower}-${component_slug}/reports/function-classification-auditor.md`, `${planning_dir}/code-quality/${wu_lower}-${component_slug}/reports/push-pull-auditor.md`, and `${planning_dir}/code-quality/${wu_lower}-${component_slug}/aggregate-code-quality.md`. The gate-set dispatches or verifies the whole touched component rows before that component closes into the aggregate diff consumed by downstream review. This per-component fanout does NOT replace Phase 8 PR-review aggregate gates.
- Rule: Phase 6 per-component code-quality auditors run outside the worktree. The gate-set computes or consumes `code_quality_dispatch_dir` as the nearest existing common ancestor of absolute `worktree_path` and `planning_dir`, then dispatches each code-quality child with `agents -m <model> -p ${code_quality_dispatch_dir} -f <prompt>`. Each child prompt must pass absolute `worktree_path`, `contract_path=${planning_dir}/contracts/${wu_lower}-${slug}.md`, and `proposal_path=${planning_dir}/proposals/${wu_lower}-${wu_id}.md`. Every selected child auditor must read the Step 6a contract before scoring declared roles, adapter declarations, intrinsic-surface declarations, proof, or validation context; missing or unreadable contract evidence is `BLOCKED`, not generic scoring fallback.
- Rule: per-component code-quality verdict semantics. The per-component code-quality auditor fanout is pipeline-callable and blocking. Only `LOW` per-component aggregate verdict allows that component to close into the aggregate diff consumed by Phase 7 CodeRabbit. `MEDIUM` and `HIGH` per-component verdicts block component closure, trigger remediation/revise from current evidence, and are never accepted as a residual, a `NEEDS_INPUT` user-disposition choice, or a stable allow-advance state. After two consecutive non-converging remediation rounds, the WU decomposes autonomously per `~/ai/conventions/code-quality.md` § `Oscillation signals WU-too-large`: decompose is not `NEEDS_INPUT`-mediated, the orchestrator records the oscillation evidence in `${planning_dir}/audit-history.md`, files the smaller sub-tickets through the project's ticket operator, and returns `DECOMPOSED` to the root caller instead of attempting a third remediation pass.
...
- Rule: Halt with `multi_layer_derivation_violation` when a Step 6c derivation output includes more than one component depth layer in a single derivation pass. The current level must not hand multi-layer derivation output forward as valid Phase 7 or child-recursion evidence. Orchestrator-runtime detection and emission live in `agents/implementation-pipeline-orchestrator.md` Step 6c (the multi-layer derivation acceptance check).
- Rule: when neither trigger arm fires, no derivation record is required and Phase 7 proceeds unchanged.
- Rule: when either trigger arm fires and no internal component split is accepted, record an evidence-bearing no-split / rejection subsection before Phase 7. When halt logic fires, that subsection is the `HaltRecord` named below.
- Rule: for docs/workflow WUs whose guard is `~/ai` markdown/operator/workflow/convention/routing/anchor structural-verification, regression-guard responsibility routes actively through WRITE eval-spec authoring under `~/ai/conventions/evals.md`; Step 6b must not add markdown structural shape tests, pytest-shaped assertions, or one-off verifier scripts as behavior evidence.
...
- Rule: the post-prototype derivation record is `LevelComponentSet`; in addition to the derivation-record fields above, it requires `layer_level_id`, `component_pair_refs[]`, `integration_test_refs`, and `coverage_summary`. `component_pair_refs[]` is the integration-test-targeted subset of pairs derivable from accepted components plus `neighbor_claims`. `coverage_summary` must distinguish "no interacting pairs at this layer" from an interacting pair with missing test refs or missing integration coverage.
- Rule: `LevelComponentSet` trace text may summarize the pairing source as component_pair_refs[]derived from accepted components plus `neighbor_claims` when linking the accepted component inventory to required integration coverage.
- Rule: Coupling-driven component-set evolution. After Step 6c has passing level evidence and a candidate or accepted current-layer component inventory exists, coupling examination may change that inventory in either direction: it may merge component elements together when the evidence shows they are one component, or it may introduce an abstraction component when the evidence shows shared dependencies should be decoupled through an intermediate component. Coupling-driven changes stay at the current `level_id`; nested sub-components, grandchild components, and multi-layer hierarchy still route through recursive child entry, not through this rule.
- Rule: `CouplingDecision` records component-set evolution from coupling evidence. It is recorded adjacent to the current `LevelComponentSet` evidence, or adjacent to the candidate component inventory when no `LevelComponentSet` has been accepted yet. Required fields are `level_id`, `source_level_component_set_ref`, `candidate_component_inventory_ref` when no accepted `LevelComponentSet` exists, `coupling_audit_report_refs`, `cohesion_audit_report_refs` when the decision depends on per-component organization evidence, `verdict`, `changed_component_refs`, `removed_or_retired_refs`, `new_abstraction_component_refs`, `integration_test_refs`, `rationale_evidence_refs`, and `defer_to_prototype_risk`.
- Rule: `CouplingDecision.verdict` is exactly one of `merge_components`, `introduce_abstraction_component`, or `no_change`. `merge_components` means the current-level component inventory collapses the affected component refs into one component. `introduce_abstraction_component` means the current-level component inventory adds a new abstraction component to decouple shared dependencies. `no_change` means coupling/cohesion evidence was considered and the current component inventory remains unchanged. A `no_change` verdict does not bypass `HaltRecord` or no-split evidence when existing derivation or halt rules require them.
- Rule: Evidence-bearing halt rule: a level halts only when candidate splits earn granularity under none of the `halt_basis` options: `clarify contracts`, `reduce accidental coupling`, `expose a design challenge`, `improve evidence`, `lower meaningful risk`. The `halt_basis` field records option-level evidence showing why each listed option was unsatisfied. The canonical halt record path is `${planning_dir}/risk/${wu_lower}-halt-record.md`. The implementation-pipeline orchestrator checks this rule procedurally per `~/ai/agents/implementation-pipeline-orchestrator.md` § `#### Phase 6 halt-state transition gate`.
- Rule: `HaltRecord` required fields are `halt_basis`, `component_candidates_considered`, `evidence_refs`, and `residual_risk_refs`. `component_candidates_considered` is required and lists considered or rejected split candidates so the halt is replayable.
- Rule: an interacting component pair listed in `component_pair_refs[]` without corresponding `integration_test_refs` is not non-applicability and must not be silently skipped; it halts with violation code `integration_test_missing` and requires replayable missing-pair evidence naming which pair and what expected coverage was missing.
- Rule: Halt verdict-resolution rule: halt-by-omission must not override a split or revise verdict. An accepted halt that conflicts with an overlay verdict cites the conflicting overlay verdict evidence and the evidence justifying halt; unresolved accepted conflicts are recorded as Decision Recording entries named `auditor_verdict_conflict_residual`.
- Rule: Process-tree review: after Step 6c completes and before Phase 7, dispatch `apply-gate-set(caller_mode=implementation-phase-6)`. The operator-owned expected-process manifest and process-tree audit #2 must prove separate Step 6b and Step 6c invocations, timing order, Step 6b output index presence, Step 6b output paths, ACR-247 side-channel consumption evidence carried by the expected-process manifest for those outputs, test-contract alignment, prototype risk review, per-component code-quality, the derivation record or no-split / rejection subsection when a derivation trigger fires, `HaltRecord` or halt/no-split evidence when halt logic fires, procedural handoff evidence for Step 6c-discovered obligations, and `CouplingDecision` evidence with rerun/current `integration_test_refs` when coupling examination changes or confirms the current component inventory. Missing, malformed, mismatched, stale, late, wrong-scope side-file evidence, or a currentness mismatch under `~/ai/conventions/apply-gate-set-currentness.md` § `Currentness key schema` and § `Row-level re-verification` is blocking. A returned `BLOCKED`, `NEEDS_INPUT`, `MEDIUM`, `HIGH`, `STALE_REFUSAL`, or process-tree blocking row prevents Phase 7; the affected subtree is rerun or repaired before downstream consumption.
...
The implementation-pipeline orchestrator runs readiness checks, acquires and verifies one draft PR, then calls `~/ai/tools/coderabbit_review_driver.py` directly for optional PR-mode CodeRabbit review. The script owns @mention triggering, review-ID baselines, exact-head generation polling, authoritative rate-limit/capacity evidence, review/comment normalization, open-finding retrieval, exact-comment replies, and per-comment body persistence. The repository-level `coderabbit` label is only an installation marker; applying it to a PR suppresses CodeRabbit and is not a trigger.

- Pre-CodeRabbit readiness refuses to advance when inherited pending production behavior tests are missing, still marked, unrun, failing, or silently replaced; it also checks integration-test evidence for same-layer interacting component pairs and `PrototypeSwapRecord` / explicit non-applicability.
...
- Trigger mode criterion: use `incremental` for the normal single PR review. Use `full` only for code-audit or mass-cleanup workflows whose declared review target is whole files rather than the latest diff, such as file-by-file risk-assessment cleanup passes.
...
- The driver owns exactly one CodeRabbit review generation per PR: snapshot CodeRabbit review IDs before its trigger, resume or suppress the outstanding generation, enforce the 300-second minimum cadence between driver-owned polls, persist each thread's complete ordered history, dispatch separate fixer invocations per actionable conversation, preserve each structured outcome, push fixes, post exact replies, and continue each conversation independently without a follow-up review generation.
- Only a new CodeRabbit pull-request review ID outside the trigger baseline whose `commit_id` equals `expected_head_oid` and whose `submitted_at` is no earlier than `triggered_at` produces `generation_result=REVIEW_COMPLETED`. Aggregate `reviewDecision`, summaries, trigger/completion acknowledgements, and checks are informational and cannot complete the review. After completion, later branch changes and pipeline reruns reuse the persisted result and must not request a follow-up review.
- `generation_result=REVIEW_COMPLETED` freezes the one generated review even after fixer pushes. A pushed fix or exact reply readback is intermediate evidence; each in-diff conversation remains open until its exact GraphQL thread reports `isResolved=true`. A newer CodeRabbit reply on an unresolved thread is pushback and re-dispatches only that conversation with its updated history.
- `terminal_reason=approved` is accepted completion only when every scoped finding is resolved and a CodeRabbit review with state `APPROVED` is bound to the exact final head. A prior-head approval is history only. Persist `single_review_completion` with the resolved thread IDs and approval review identity; never request another review generation.
- A trigger-bound authoritative rate-limit comment produces `generation_result=RATE_LIMITED_NO_REVIEW`; a passing `Review rate limited` check alone does not. The driver persists an in-flight marker before posting `@coderabbitai rate limit`, reconciles interrupted POSTs before any retry, posts at most once per capacity-query generation, persists the exact bound response and provider-reported guidance, and never infers capacity from elapsed time or the fair-usage table.
- While a trigger is outstanding, provider-reported capacity is unavailable, one-review-at-a-time is active, or generation evidence is blocked, the driver suppresses another trigger. `RATE_LIMITED_NO_REVIEW` and `BLOCKED` are non-success and prevent Phase 8 advance.
- `open-findings` is the authoritative read-only unresolved-finding inventory for in-diff and outside-diff findings; each item carries exact review/comment/thread identities, URL, a provider-bound in-diff head OID or null outside-diff head OID, resolution, and persisted body path without rewriting poll deltas or generation state. Local outside-diff reply dispositions are bound to the replied body revision, so provider edits reopen the finding.
- `needs_caller_decision=true` surfaces `deferred` per-comment outcomes to the root caller without re-triggering; unresolved rejection is not an outcome.
- The orchestrator must not compose its own trigger/poll/capacity/dispatch loop, must not request a second CodeRabbit review, must not use aggregate `reviewDecision`, inline `gh pr view ... statusCheckRollup`, or ad hoc GitHub endpoint polling, must not inline CodeRabbit comment bodies, and must not synthesize an empty review pass.
- Phase 0 invokes `python3 ~/ai/tools/wu-session-migration phase0-init --request ${scratch_dir}/session-writes/phase0-init.json`; an existing eligible pre-Phase-3 session whose selected ticket contract changed invokes `python3 ~/ai/tools/wu-session-migration phase0-reresolve --request ${scratch_dir}/session-writes/phase0-reresolve.json` and validates its closed readback. Every request declares `R=${planning_dir}/..` and exact index `R/sessions.active-wake.json`; direct composition uses top-level project planning tree `P` as `R`, while feature direct/refactoring composition uses normalized `F/routes` as `R`. The writer derives `P` for same-project scratch containment and never searches for a nearer or aggregate index. An accepted inherited-estimate answer invokes `python3 ~/ai/tools/wu-session-migration cold-start-disposition-bind --request ${scratch_dir}/session-writes/cold-start-disposition-bind.json` and passes exact readback before the separate terrain/risk/defer gate. Completed Phase 3 estimate evidence invokes `python3 ~/ai/tools/wu-session-migration phase3-bind --request ${scratch_dir}/session-writes/phase3-bind.json` and passes exact readback before Phase 4. All pre-PR operations use `row_identity=null`, write only `session.json`, and hold the complete active index plus bound artifacts as read-only guards. Phase 7 writes `${scratch_dir}/pr-url.txt` and `${scratch_dir}/pr-number.txt`, records `pr_open_base_sha`, leaves `pre_merge_base_sha` unresolved, and invokes `python3 ~/ai/tools/wu-session-migration phase7-upsert --request ${scratch_dir}/session-writes/phase7-upsert.json`. Phase 9 invokes `python3 ~/ai/tools/wu-session-migration phase9-update --request ${scratch_dir}/session-writes/phase9-update.json`. Each closed request binds exact source identities and complete manifest/index projections; operators do not reproduce the transaction algorithm or write targets directly. The historical `sessions.index.json` is not a wake input, and Phase 9 reuses the exact PR/row identity.
...
- The Phase 8 gate-set inventory is test-audit review, multi-concern PR review, justification PR review, commit hygiene, Phase 8 actual-diff code-quality, verification-plan review with `verification_plan_excerpt` and `behavior_claim`, separate validation-integrity runtime-claim transport where applicable, supported-surface inventory-resolution while ACR-286 remains open, currentness rechecks against prior joins, `${planning_dir}/risk/phase-8-join-manifest.json`, and process-tree audit #3.
...
- Rule: the Phase 8 code-quality rows carry the Phase 3 `verification_plan_excerpt` and `behavior_claim` separately from the post-change `runtime_claim` and actual executed evidence. Validation-integrity and verification-plan review remain required where their distinct contexts apply.
...
- Final halt-on-missing: on a missing/invalid/inconsistent calibration block or invalid `estimate_comparison_comment_ref`, Final halts and must not post the close-comment. Final verification accepts `<id | url | none>`, rejects `path`, and treats none valid only when `ticket_system: jira`. On success, Final close-comment includes inherited/refined/actual values inline on both backends.

```worked example
Phase 8 worked example

inherited_story_point_estimate: 3
refined_story_point_estimate: 5
actual_story_points: 8
actual_capture_method: closer-best-effort
estimate_delta_narrative: inherited=3; refined=5; actual=8; delta_refined_to_actual=3; over_2x_inherited=true
...
- actual_story_points: 8
- actual_capture_method: closer-best-effort
- actual_estimate_rationale: closer-best-effort: closure scope landed broader than the refined estimate assumed.
- estimate_delta_narrative: inherited=3; refined=5; actual=8; delta_refined_to_actual=3; over_2x_inherited=true
...
- trigger: an auditor, reviewer, or implementation step identifies existing-code structural refactor work that is outside touched-file/component ownership, or inside touched ownership but too large for the current WU, and material to continuing safely.
- pause evidence: record the current ticket id, branch, head SHA, worktree path, dirty status, failing or residual report paths, and the reason the refactor is outside touched ownership or requires decomposition.
...
- touched-file/component gate failures: this pause path does not alter touched-file/component ownership; current-WU `MEDIUM` or `HIGH` code-quality findings still revise, remediate, or decompose under the existing LOW-only rules.
...
Accepted halt residuals and accepted `auditor_verdict_conflict_residual` entries follow this same Decision Recording rule when they do not collapse the approved net-value case.

Decision Recording, generic residual recording, and Phase 7 / Phase 8 residual / value-zero review language do NOT authorize residual acceptance of Phase 4 code-quality, Phase 6 prototype-risk, or per-component code-quality `MEDIUM` or `HIGH` verdicts. Those gate verdicts revise, remediate, or decompose per `~/ai/conventions/code-quality.md` § `Disposition policy` and § `Oscillation signals WU-too-large`; they are never recorded as residuals. Generic residual recording remains valid for non-gate residual classes only (bounded test-verification residuals in `risk/NN-test-residuals.md`, process-provenance uncertainty per the Phase 6 firstness paragraph above, accepted halt residuals, accepted `auditor_verdict_conflict_residual` entries, and Phase 8 value-zero termination evidence).
...
- `~/ai/agents/coderabbit-operator.md`
