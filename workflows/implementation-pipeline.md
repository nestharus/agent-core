...
- A trigger-bound authoritative rate-limit comment produces `generation_result=RATE_LIMITED_NO_REVIEW`; a passing `Review rate limited` check alone does not. The driver posts `@coderabbitai rate limit` at most once per capacity-query generation, persists the exact bound response and provider-reported guidance, and never infers capacity from elapsed time or the fair-usage table.
...
- `open-findings` is the authoritative unresolved-finding inventory for in-diff and outside-diff findings; each item carries exact review/comment/thread identities, URL, head OID, resolution, and persisted body path.
