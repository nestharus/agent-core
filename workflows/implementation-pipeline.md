...
- Rule: RCA-track evidence is **verbatim output, not summary**. The red run is captured by reverting the planned-change product file(s) to the branch's base (`origin/main` or the stacked parent), running each reproduction test, and pasting the failure block into the Phase 9 evidence artifact. The green run is captured by restoring the fix and re-running the same test, pasting the pass block. "Tests passed" is not evidence; the actual test-runner or browser-runner command output is.
