...
- `terminal_reason=no_value_provided` is accepted convergence only when the findings came from a `REVIEW_COMPLETED` generation and every actionable in-diff comment in the latest iteration was assessed `review_provided_value: false`.
- `terminal_reason=review_applied` is accepted completion when the findings came from a `REVIEW_COMPLETED` generation and all useful findings were applied, pushed, and replied to as required; the final head is intentionally not sent through CodeRabbit again.
