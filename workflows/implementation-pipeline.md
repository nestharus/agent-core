...
For an enabled or legacy-capable contract, the orchestrator dispatches `${ticket_operator}` once with `task=update-estimate`, requires the operator's durable rationale and verified success contract, and records `disposition=write_verified`. Any backend, network, authorization, validation, or operator failure blocks and cannot become no-write evidence. For authoritative wrapper `false`, it does not compose, dispatch, or simulate `update-estimate` and records `disposition=no_write_policy_disabled`. The Phase 4 scope-risk prompt always receives the same complete `estimate_delta_flag`; inherited null requires `over_2x: unknown`, never `false`.
