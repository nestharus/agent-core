...
- **Review agents do not synthesize** (per `~/ai/models/roles.md`). Alignment, classify, and integrate use separate `gpt-high` invocations.
...
```
Stage 1                — problem-alignment       (gpt-high)  → problem-review.md (always), problem-surfaces.md (conditional)
  Stage 1b-classify    — problem-expansion-classify  (gpt-high)  → problem-classification.md
    Stage 1b-integrate — problem-expansion-integrate (gpt-high)     → updates problem.md + axis table
Stage 2                — philosophy-alignment    (gpt-high)  → philosophy-review.md (always), philosophy-surfaces.md (conditional)
  Stage 2b-classify    — philosophy-expansion-classify (gpt-high) → philosophy-classification.md, philosophy-decisions.md (conditional)
...
- Each Stage's "did the proposal align?" question is a **model gate** owned by the alignment operator (gpt-high). No human gate within the cycle.
