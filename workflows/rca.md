...
| 2 | Root Cause | fresh `gpt-high` RCA dispatch | `${planning_dir}/rca/<failure-id>.md` | Phase 3 |
| 3 | Best Appropriate Fix | fresh `gpt-high` fix-decision dispatch | `${planning_dir}/rca/<failure-id>-fix-decision.md` | Phase 4 |
| 4 | Best Way To Apply | fresh `gpt-high` application-plan dispatch | `${planning_dir}/rca/<failure-id>-application-plan.md` | Phase 5 |
| 5 | Apply | fresh `gpt-high` apply dispatch | `${planning_dir}/rca/<failure-id>-applied.md` | Phase 6 |
...
The orchestrator dispatches the test writer with `agents -m gpt-high -p ${worktree_path} -f <prompt-file>` or `agents -m gpt-high -p ${worktree_path} -f <prompt-file>` as a fresh invocation. The prompt must not expose Phase 5 implementation details because no implementation exists yet.
...
Phase 2 identifies the root cause only. Dispatch a fresh root-cause prompt with `agents -m gpt-high -p ${worktree_path} -f <prompt-file>` and require it to read the failing test evidence or `${planning_dir}/repro/<failure-id>.md`, inspect the repository read-only, and write `${planning_dir}/rca/<failure-id>.md`.
...
Phase 3 selects the best appropriate fix from the root-cause artifact. Dispatch a fresh fix-decision prompt with `agents -m gpt-high -p ${worktree_path} -f <prompt-file>` and require it to read `${planning_dir}/rca/<failure-id>.md`, compare viable fixes, and write `${planning_dir}/rca/<failure-id>-fix-decision.md`.
...
Phase 4 plans how to apply the selected fix. Dispatch a fresh application-plan prompt with `agents -m gpt-high -p ${worktree_path} -f <prompt-file>` and require it to read the root-cause and fix-decision artifacts, inspect local code as needed, and write `${planning_dir}/rca/<failure-id>-application-plan.md`.
...
Phase 5 applies the Phase 4 plan through a fresh apply prompt with `agents -m gpt-high -p ${worktree_path} -f <prompt-file>`. The apply invocation edits `${worktree_path}` only according to `${planning_dir}/rca/<failure-id>-application-plan.md` and writes `${planning_dir}/rca/<failure-id>-applied.md`.
