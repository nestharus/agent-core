...
workflow_dispatch_contract:
  ...
  inputs:
    ...
    - "Phase 4/8 plan context requires the complete proposal_path, verification_plan_excerpt, and behavior_claim tuple and rejects partial context; other optional Phase 4 evidence: problem_map_path, risk_profile_path, runtime_claim, and wu_id"
...
- `worktree_path=<absolute-path>`: required for pipeline-callable invocations; points to the active worktree used for source and relative evidence resolution. Auditors must not assume their current working directory is the worktree.
...
- `scratch_dir=<path>`: required for pipeline-callable invocations; stores prompts and logs.
- `planning_dir=<path>`: required for pipeline-callable invocations; stores durable reports, findings, manifest, and aggregate output.
...
Phase 4/8 plan context is accepted only as the complete `proposal_path`, `verification_plan_excerpt`, and `behavior_claim` tuple. If any member is present or plan context otherwise applies, all three are required and the `verification-plan-reviewer` child prompt must carry all three; partial context is `BLOCKED:partial-verification-plan-context`.
...
In Phase 6 per-component mode, the dispatch manifest must record `code_quality_dispatch_dir` and every child dispatch must use `agents -m <model> -p ${code_quality_dispatch_dir} -f <prompt> 2>&1 | tee <log>`. The child prompt must pass absolute `worktree_path`, `contract_path`, and `proposal_path`. Missing, unreadable, blank, or malformed `contract_path` is `BLOCKED:unreadable-contract-path`; it is never permission to score from generic judgment.
...
| ACR-254 | verification-plan-reviewer | gpt-xhigh | `${scratch_dir}/code-quality/${slug}/prompts/verification-plan-reviewer.prompt.md` | `${scratch_dir}/code-quality/${slug}/logs/verification-plan-reviewer.log` | `${planning_dir}/code-quality/${slug}/reports/verification-plan-reviewer.md` | context-dependent | Phase 4/8 plan context: complete `proposal_path`, `verification_plan_excerpt`, `behavior_claim` tuple plus `worktree_path`; RCA: fix-decision artifact with `## Verification plan`; Phase 6 also `contract_path` |
...
- Required inputs: `mode=phase-3-proposal` for implementation-pipeline proposals or `mode=rca-fix-decision` for RCA fix-decision artifacts, plus `proposal_path`, `report_path`, and `worktree_path`. Phase 4/8 plan context additionally requires `verification_plan_excerpt` and `behavior_claim`, and the child prompt must carry the complete three-field tuple.
- Phase 6 required inputs: `contract_path` and `proposal_path`; the auditor must read both before scoring.
