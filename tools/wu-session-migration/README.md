...
For `phase3-bind` only, the persisted resolved operator and optimized contract are producer-time identities. Their digests must match regular tracked blobs at the manifest's exact full `branch_out_sha` commit and canonical repository-contained paths; all request artifacts retain their live filesystem identity guards. Unknown or non-commit objects, missing or non-blob paths, symlinks, gitlinks, and digest mismatches fail before mutation.
