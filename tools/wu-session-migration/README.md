...
Post-write evidence for `cold-start-disposition-bind` and `phase3-bind` uses the closed `wu-session-pre-pr-write-readback-v1` schema. `validate-pre-pr-readback --readback <path>` binds that evidence to the immutable writer request and independently verifies the before/after manifest projection, exact changed keys and Phase 3 history append, current artifact identities, unchanged active-index identity and rows, absence of a synthesized row, and absence of a retained journal. A literal `verdict=PASS` does not bypass these checks.
