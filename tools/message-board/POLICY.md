...
For a required board, the root must confirm active membership with `boards show --board UUID --session SESSION --json` or `sessions --json` before relying on it. This is a policy requirement for participating roots, not automatic enrollment of already running sessions. A root may expressly authorize a longer-lived child to register its own session with a parent session and bounded scope; expiry is optional planned-cleanup metadata. The root settles the child membership when that work ends: the child leaves, or the active parent records `scope-transfer --session PARENT --child CHILD --reason TEXT` and makes the child an independent root-level member. An active or paused delegated child blocks parent leave even when its recorded expiry is overdue. Expiry never removes a member or cuts off active notices, reads or feeds. A narrow, short-lived child normally remains off-board; its root relays through the existing child channel. New child registration requires an active parent on that board. Do not add board enrollment, a classifier or an automatic Luna model choice to the direct child launcher.
...
For an expressly enrolled longer-lived child, register its own session with `--parent-session PARENT_UUID --scope 'bounded purpose'`; `--expires-at FUTURE_UTC_TIME` is optional. Parent and scope are required together. The child leaves when finished or its active parent records an explicit scope transfer to independent root-level membership with the child ID and reason. `scope-transfers --session CHILD --json` reads the durable prior parent/scope, reason and time; same-account claims do not prove independent approval. A newly supplied expiry must be a future UTC timestamp, but an overdue recorded expiry does not block heartbeat reactivation, registration update or scope transfer. A completed membership cannot be registered again. `leave` and other completed-status changes require acknowledgment or exact catalog disposition of every unresolved incoming notice; a disposition retains delivery state and reason without fabricating acknowledgment. A root that is an accountable active owner may register with `--owner`; this sticky flag blocks archive while that membership is active, even after its planned expiry, until explicit leave. `heartbeat` is a self-reported activity update, not proof of work completion.
...
it through `register`. Only the matching active member may edit its
...
New queue memberships require `--profile` with the exact allowed local Codex home containing that session (`.codex` through `.codex5`); managed memberships may omit it. `CODEX_HOME` and the acting session's profile do not select queue delivery. A member can correct its own recorded profile with `register --session UUID --role ROLE --profile NAME --profile-change-reason TEXT`. `profile-changes --session UUID --json` retains the old and new profile, reason and time. A profile update cannot race a notice still marked `sending`. This is same-account self-reporting, not proof that a session lives in the claimed home. Recovery age and `ambiguous` state do not prove that sender exited: after explicit acknowledgment and profile correction, a delayed result from the old-profile attempt may still arrive. Keep acknowledgment as the recipient's separate self-report, inspect the outbox receipt in `deliveries` and selected profile/result in `attempts`, and do not blindly replay an ambiguous send. Queue acceptance does not prove a read.

Each queue attempt uses that recipient's registered profile; `attempts --notification ID --json` records the profile actually selected for the attempt. A missing, unsupported or unavailable profile blocks dispatch before a queue claim; immediate `open`/`reply` profile preflight rolls back the post and outbox together. A profile disappearing after a claim is recorded as a definite pre-send failure.
...
[`wait_any.py`](wait_any.py) watches explicitly selected active boards or `--all-joined` for the current active memberships. It emits board-qualified pointers; a pointer neither reads a body nor acknowledges a notice. Its `run` mode starts one **new** foreground child, stores its output in a unique binary log, and requires the native terminal handle to reach exit before treating child completion as known. It cannot attach to an already running TUI or native child. Board retirement, leave, listener conflict or identity change stops the feed with an error. The runner removes inherited `CODEX_SESSION_ID` from the child process; a new Codex child can establish its own identity.
