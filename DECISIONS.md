...
## 2026-05-16 — ACR-247 — Halt → bootstrap-exception resume → implementation worktree pivot

- WU: ACR-247 (Agent-runner runtime side-channel for load-bearing log evidence).
- Phases: Phase 4 R1→R5 (audit-risk oscillation halt) → Resume under bootstrap exception → Phase 5/6/7 advance under bootstrap-ratified audit-risk → Phase 8 PR-review.
- **D4 — Initial halt as `BLOCKED:auditor-strictness` per AGE-116 disposition**: Phase 4 audit-risk gate produced a third non-overlapping new-finding family on pre-existing proposal content across R1→R4 (R1 model-attestation HIGH → R2 LOW → R3 broad anti-scope MEDIUM → R4 A9/A10 assumption-evidence MEDIUM on assumption-register text unchanged from R3). The R3 audit-history entry pre-declared the AGE-116 halt criterion; R4 satisfied it. The dispatch brief explicitly authorized this halt for "ACR-246 territory".
- **D5 — Bootstrap-exception ratification (manager authorization)**: root replied with the exact quote `we are bootstrapping the consume rule and have no way to follow it right now.` ACR-247 IS the consumed: rule's fix-cluster (sibling to ACR-246 which is the auditor scope-tightening fix-cluster); the two ARE the architectural fixes for the very rules that block them; mutual fix-cluster deadlock; bootstrap exception is the canonical answer to mutual fix-cluster deadlock.
  - **Narrow scope**: (1) Phase 4 audit-risk gate (R4 MEDIUM) RATIFIED for this WU only; (2) Phase 6 Step 6c consumed: rule conditionally ratified if Step 6c hits the rule's wall for itself.
  - **LOW-only stays in force** on every other gate (scope, shortcut, supported-surface, code-quality, all Phase 6 sibling gates, all Phase 8 PR-review gates).
  - The justification is structural non-applicability of the rule to its own fix, NOT residual-acceptance via precedent citation; ACR-242 is honored.
- **D6 — Implementation worktree pivot to `~/ai/`**: dispatch brief routed `worktree_path` to `agent-runner`, but ACR-247's Phase 3 R5 proposal modifies `~/ai/` files (no agent-runner Rust changes; Design 1 explicit). Pivoted product changes to `/home/nes/projects/ai/worktrees/acr-247-runtime-consumed-side-channel/` (the canonical `~/ai/` worktree pattern, matching sibling `acr-246-audit-the-auditor`). Planning_dir stays on agent-runner per dispatch. Phase 9 PR target: `~/ai/`. Agent-runner branch carries no product changes (planning/decisions only).
- **Result of Phase 4 R5**: scope/shortcut/supported-surface/code-quality (cohesion+coupling)/aggregate all LOW; audit-risk MEDIUM bootstrap-ratified. Phase 4 join manifest published with `bootstrap-exception-audit-risk` row marked RATIFIED. Process-tree audit #1 PASS.
- **Phase 6**: Step 6a contract authored; Step 6b R1→R4 produced 11 fixtures + runnable `eval.py` with `--strict` mode; alignment review reached ALIGNED on R4; Step 6c modified 5 operator/workflow/convention/eval-spec files + added `workflows/step6c-consumption-side-file.md` + `workflows/step6c-consumption-side-file/projection.sh` + the runnable eval. `python3 eval.py --strict` exits 0 with all 11 fixtures PASS. Process-tree audit #2 PASS. Phase 6 halt-record / prototype-swap-record / derivation-status / multi-layer-check all carry explicit `non-applicable` / `no-trigger-fired` per the spec's missing-artifact-is-not-equivalent-to-non-applicability rule.
- **Phase 7**: CodeRabbit retired 2026-05-15; 3 readiness gates pass-no-trigger / pass-non-applicable; pass-through to Phase 8.
...
### ACR-209 — Bootstrap exception ratification (Phase 6 per-component fanout extension)

- WU: ACR-209 (route ~/ai structural guards to WRITE eval specs)
- Phase: Phase 6 per-component code-quality fanout post-Step-6c (R2 verdict HIGH, same architectural-debt findings Phase 4 R6 already accepted as LOW common-interface proof)
- Decision: Apply bootstrap-exception ratification per `~/ai/conventions/code-quality.md` § `Bootstrap exception`, extended to the Phase 6 per-component fanout (same shape as AGE-132 + AGE-137 precedent). Root explicitly overrode the brief's `NO bootstrap-exception` anti-scope for ACR-209 specifically; this DECISIONS entry is the ratification record per `~/ai/conventions/code-quality.md` § `Bootstrap exception` four-condition check.
- Four-condition check (Phase 6 extension):
  1. **Primary deliverable IS the metric fix.** ACR-209's whole purpose is routing `~/ai` markdown/operator/workflow/convention/routing/anchor structural-verification work to WRITE-state eval specs at `evals/<slug>/eval.md`. The audited metric (touched-file code-quality compliance for the routing-edit surfaces) IS exactly the thing ACR-209 fixes via the proposal `## Supported-surface track` edits + the new WRITE-state eval spec. The post-Step-6c HIGH findings are about the metric being audited, not about an unrelated debt.
  2. **Non-LOW findings are intrinsic-lockstep with the refactor.** The 5 per-component post-Step-6c findings (PP-001 state.db, PP-002/PP-003 trace pull, COUPLING-001 aggregate raw count, COUPLING-002 evals.md non-subordinate refs) are about the same architectural debt the routing-edit deliverable confronts. The refactor itself produces the post-cleanup surface; demanding pre-refactor LOW on the metric the WU is fixing creates a chicken-and-egg block. Same shape as AGE-132/AGE-137 bootstrap precedent.
  3. **Phase 8 PR-review re-evaluates code-quality at the PR-diff level**, replacing retired Phase 7 CodeRabbit. Phase 8 four PR-review gates (test-audit, multi-concern, justification, commit-hygiene) are independent of the bootstrap-exception ratification and remain LOW-required. The downstream PR-diff gate validates the actual ship. The bootstrap-exception is local to the Phase 6 per-component fanout only.
  4. **DECISIONS ratification + ACR-253-pattern auditor non-determinism evidence cited.** Phase 4 R6 push-pull-auditor PP-006 (`code-quality/acr-209-phase-4/reports/push-pull-auditor.md`) explicitly accepted the AGE-144 v1 schema citation as LOW common-interface proof for the orchestrator's path-to-invocation matching at lines 308 and 524. The per-component post-Step-6c auditor with the SAME orchestrator file content scored the same evidence HIGH. This is auditor calibration non-determinism (same pattern as the R5 master-vs-worktree path-resolution non-determinism that was resolved by committing the worktree state). Per-component HIGH cannot converge against canonical Phase 4 R6 LOW evidence preserved at `risk/phase-4-join-manifest.json`.
- Justifying evidence:
  - `/home/nes/ai/planning/acr-209-route-structural-guards-to-eval-specs/audit-history.md` § `Round resume — COMMIT_AND_R6 disposition` (Phase 4 R6 LOW result + Process-tree audit #1 PASS + Phase 6 alignment ALIGNED + post-Step-6c R1/R2 HIGH analysis)
  - `/home/nes/ai/planning/acr-209-route-structural-guards-to-eval-specs/risk/phase-4-join-manifest.json` (Phase 4 R6 canonical evidence; all 5 gates LOW)
  - `/home/nes/ai/planning/acr-209-route-structural-guards-to-eval-specs/risk/acr-209-phase-4-process-tree.md` (Process-tree audit #1 PASS)
  - `/home/nes/ai/planning/acr-209-route-structural-guards-to-eval-specs/code-quality/acr-209-phase-4/reports/push-pull-auditor.md` PP-006 (R6 LOW common-interface proof for orchestrator trace pull)
  - `/home/nes/ai/planning/acr-209-route-structural-guards-to-eval-specs/code-quality/acr-209-post-step6c/aggregate-code-quality.md` (R2 HIGH; same orchestrator file content, opposite verdict — auditor non-determinism)
  - `/home/nes/ai/planning/acr-209-route-structural-guards-to-eval-specs/alignment/acr-209-tests-contracts.md` (Phase 6 alignment ALIGNED)
  - Question artifact: `/home/nes/ai/planning/acr-209-route-structural-guards-to-eval-specs/.scratch/questions/q-d3ce2362-72fa-43be-93fa-8c5158e7f933.question.json`
  - Answer artifact: `/home/nes/ai/planning/acr-209-route-structural-guards-to-eval-specs/.scratch/questions/q-d3ce2362-72fa-43be-93fa-8c5158e7f933.answer.json` (selected_option_id=ADVANCE_TO_PHASE_8, answered_by=root)
- Precedent context (NOT residual-acceptance authority): AGE-132 + AGE-137 used the same four-condition bootstrap-exception pattern for refactor work where the audited metric is the thing being fixed. ACR-253 is the auditor-calibration-non-determinism marker. This DECISIONS entry is the ratification record per `~/ai/conventions/code-quality.md` § `Bootstrap exception`.
