...
proof_acceptance: canonical-header-first-unique-report-PASS-producer-binding-current-and-final-stdout-PASS
...
The prompt requires canonical first line `# Domain Research: <domain>`. After notification, extract to distinct `$WORK_DIR/result-<domain>.md`. After dispatch, freeze the actual marker UUID and distinct prompt/log/output hashes, capture a current root trace, and require an independent process-tree auditor with separate `.log` and child-owned `.md`, a canonical header-first report whose one verdict is `Verdict: PASS`, an exact producer-owned report/root/expected/trace/companion machine binding, and successful envelope payload final line `PASS`. Omitted, wrong-model, stale, aliased, or hash-mismatched domain children cannot contribute to posting. Prepare each passing domain finding as a separate comment payload with clear framing; do not post before Phase 8.
...
Snapshot existing review IDs from `gh api --method GET "repos/${REPO}/pulls/${PR}/reviews"` before mutation. Only then post the prepared review with `gh pr review "$PR" --repo "$REPO" --body-file "$WORK_DIR/review-body.md"` and the previously selected `--request-changes` or `--comment` mode. Query the new review ID and URL only from `gh api --method GET "repos/${REPO}/pulls/${PR}/reviews"`, requiring exactly one new identity for the authenticated author and prepared body.

For each prepared recommendation/domain/follow-up payload, snapshot existing issue-comment IDs from `gh api --method GET "repos/${REPO}/issues/${PR}/comments"`, post with `gh pr comment "$PR" --repo "$REPO" --body-file "<prepared-path>"`, and query its created ID and URL only from `gh api --method GET "repos/${REPO}/issues/${PR}/comments"`, again requiring exactly one new identity with the expected author/body. Every `gh pr review` and `gh pr comment` mutation carries both the exact PR and `--repo "$REPO"`; no mutation or identity query may infer a PR or repository from the detached checkout or current branch.
...
proof_acceptance: "canonical header-first unique report Verdict: PASS, current producer-owned machine binding, and final stdout PASS"
