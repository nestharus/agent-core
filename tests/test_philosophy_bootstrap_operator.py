"""Structural tests for the NES-234 philosophy-bootstrap operator."""

import re


OPERATOR_RELATIVE_PATH = "agents/philosophy-bootstrap.md"
...
def test_philosophy_bootstrap_file_exists(repo_root):
    assert _operator_path(repo_root).exists()


def test_philosophy_bootstrap_frontmatter_exact_keys(repo_root):
    frontmatter = _parse_frontmatter(_operator_text(repo_root))

    assert set(frontmatter) == REQUIRED_FRONTMATTER_KEYS


def test_philosophy_bootstrap_frontmatter_model_is_gpt_high(repo_root):
    frontmatter = _parse_frontmatter(_operator_text(repo_root))

    assert frontmatter["model"] == "gpt-high"


def test_philosophy_bootstrap_frontmatter_output_format_is_empty(repo_root):
    frontmatter = _parse_frontmatter(_operator_text(repo_root))

    assert frontmatter["output_format"] == ""


def test_philosophy_bootstrap_description_non_empty_and_period_terminated(repo_root):
    frontmatter = _parse_frontmatter(_operator_text(repo_root))
...
def test_philosophy_bootstrap_required_body_sections_in_order(repo_root):
...
def test_philosophy_bootstrap_required_inputs_mentions_all_named_inputs(repo_root):
...
def test_philosophy_bootstrap_stop_conditions_mentions_required_tokens(repo_root):
...
def test_philosophy_bootstrap_anti_scope_names_contract_boundaries(repo_root):
    anti_scope = _section_after_h2(_operator_text(repo_root), "Anti-scope")

    assert re.search(r"does\s+NOT\s+change\s+`?problem\.md`?", anti_scope, re.I)
    assert re.search(r"does\s+NOT\s+dispatch\s+`?proposer`?", anti_scope, re.I)
    assert re.search(
        r"does\s+NOT\s+execute\s+Stage\s+2/2b\s+sub-agents",
        anti_scope,
        re.I,
    )
    assert re.search(
        r"requires\s+a\s+readable\s+`?problem\.md`?.{0,120}will\s+NOT\s+proceed",
        anti_scope,
        re.I | re.DOTALL,
    )
