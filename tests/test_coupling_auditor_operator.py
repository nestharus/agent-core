...
def test_threshold_bands_named():
    ...
    assert _has_band(metric_binding, "MEDIUM", r"(?:>=\s*3|≥\s*3|3\+)"), (
        "coupling MEDIUM band must include >=3, ≥3, or 3+"
    )
    assert _has_band(metric_binding, "HIGH", r"(?:>=\s*6|≥\s*6|6\+)"), (
        "coupling HIGH band must include >=6, ≥6, or 6+"
    )
...
def test_evidence_rule_stated():
    ...
    assert re.search(r"\b(?:MEDIUM|HIGH|non-LOW|non LOW)\b", text)
