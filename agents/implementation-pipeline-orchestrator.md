...
8. Interpret the final JSON:
   ...
   - `terminal_reason=review_applied` with `generation_result=REVIEW_COMPLETED` and `single_review_completion` is success only after every finding was resolved by a validated `fixed`, `replied`, or `fixed_and_replied` disposition and all required pushes/replies passed exact readback. Record both the reviewed head and final post-fix head, then proceed without requesting another CodeRabbit review.
