...
8. Interpret the final JSON:
   ...
   - `terminal_reason=no_value_provided` is convergence only when its findings belong to a `REVIEW_COMPLETED` generation; record the no-value decision and proceed unless the project has an explicit stricter policy.
