...
6. **Cross-link to the ticket system.** Compose `${scratch_dir}/prompts/${wu_lower}-phase-9-ticket-comment.md` instructing `${ticket_operator}` to perform `task=comment` on `issue_key=${ticket_id}` with a comment body (ADF for JIRA, Markdown for Linear) containing the PR URL, base branch, head branch, and a one-line summary. Dispatch via `agents -m claude-opus -p ${worktree_path} -f ${prompt} 2>&1 | tee ${log}`. The comment is the ticket-side announcement that a draft PR exists; the ticket is the source of truth for status, the PR is the source of truth for the diff.
