...
7. Run the driver-owned pass:
   ...
   The driver owns exactly one CodeRabbit review per PR. It snapshots CodeRabbit pull-request review IDs before the trigger, resumes or suppresses a persisted outstanding generation, and revalidates the provider head against the local worktree before and after every poll. Only a new CodeRabbit review ID outside that trigger baseline with `commit_id == expected_head_oid` and `submitted_at >= triggered_at` produces `REVIEW_COMPLETED`; aggregate `reviewDecision`, summary comments, trigger/completion acknowledgements, and checks cannot complete the review. The driver enforces the 300-second minimum cadence, persists normalized bodies and generation evidence, dispatches each unresolved current-head in-diff comment ID at most once, pushes fixed commits, verifies remote/provider head readback, posts exact-comment replies with identity readback, persists `single_review_completion`, and terminates. It must not request a follow-up review, including after a fix push or unchanged-head reply.
8. Interpret the final JSON:
   ...
   - `terminal_reason=review_applied` with `generation_result=REVIEW_COMPLETED` and `single_review_completion` is success after useful findings were applied. Record both the reviewed head and final post-fix head, then proceed without requesting another CodeRabbit review.
