...
Dispatch `process-tree-auditor` (`gpt-high`) against the Phase 6 subtree. The expected-process manifest must require: separate Step 6b and Step 6c invocations, timing order, output-index presence, output paths, Step 6c log evidence of consuming Step 6b outputs. When Step 6c records implementation-discovered procedural obligations, the expected process must also require procedural handoff evidence: a separate Step 6b-style procedural test-writer invocation UUID distinct from the Step 6c invocation UUID; timing order proving Step 6c discovered and recorded the obligation before that invocation ran; updated `${scratch_dir}/phase6/step6b-output-index.md` rows for each obligation naming the Step 6c evidence; and either an emitted procedural test path plus test/test-group identifier or a procedural residual entry path plus residual class. Missing procedural handoff evidence is `blocking` before Phase 7. A `blocking` verdict prevents Phase 7.
