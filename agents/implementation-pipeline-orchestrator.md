...
**ACR-310 estimate-disposition migration:** an existing session with no `estimate_mutation_policy`, operator/contract hashes, and `estimate_writeback_disposition` in `${planning_dir}/session.json` must not silently adopt this procedure. Re-enter Phase 0, resolve and hash the current selected operator and optimized contract, reread and hash the current ticket snapshot, record a migration row in session/audit history, and regenerate every downstream artifact whose currentness depends on those identities. A session blocked before Phase 2.5 or Phase 3 restarts from Phase 0; RGDEV-1078 must follow this path and must not reuse its pre-ACR-310 join manifest.
