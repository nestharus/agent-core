...
Sub-agents emit `NEEDS_INPUT:<question_artifact_path>` per `~/ai/conventions/agent-questions-and-session-graph.md`. You classify each question:

- **Procedural NEEDS_INPUT** (missing input you can supply: e.g. "where is the ticket?", "what's the worktree path?"): you answer it yourself and re-dispatch the sub-agent with the answer in its prompt. Do NOT bother the root.
- **New value question** (surfaces an unevaluated value, scope, or trade-off the orchestrator cannot decide): emit a NEEDS_INPUT to the root with the original question artifact attached. Block until answered. This is the second permitted human-gate trigger.
