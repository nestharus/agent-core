...
- **Three process-tree audits per WU.** Phase 4 join, Phase 6 join, Phase 8 join. All three are mandatory; a `blocking` verdict from any of them halts the next phase until the affected subtree is rerun or repaired.
- **Human gates are restricted.** The only two human gates in this orchestrator's flow are:
...
- **AskUserQuestion permission-denial on a human-owned value/scope question is artifact-and-halt, not defaulting.** Write `${scratch_dir}/questions/q-<uuidv4>.question.json` as JSON per `~/ai/conventions/agent-questions-and-session-graph.md`, return `NEEDS_INPUT:<absolute_artifact_path>`, and halt before any dependent action. procedural permission-denial or NEEDS_INPUT that the orchestrator can resolve from supplied inputs stays inline. Self-applying defaults on a new-value, scope, or trade-off question after `AskUserQuestion` is denied is a contract violation.
...
1. Compose `${scratch_dir}/prompts/${wu_lower}-phase-2.5-problem-map.md` instructing a `gpt-high` researcher to produce `${planning_dir}/research/${wu_lower}-problem-map.md`. Inputs: ticket. Agent infers the planned change surface from ticket text + codebase research.
...
6. **Human gate** (skipped when `skip_problem_map_gate=true`) — emit a NEEDS_INPUT question to the root asking for:
   ...
   If the runtime denies `AskUserQuestion` for this problem-map / risk-profile-acceptance / defer-question gate, the artifact path and JSON format are mandatory: write `${scratch_dir}/questions/q-<uuidv4>.question.json` per `~/ai/conventions/agent-questions-and-session-graph.md`, return `NEEDS_INPUT:<absolute_artifact_path>`, and stop before step 7. Treating permission-denial as a declined gate or proceeding with defaults is forbidden. `skip_problem_map_gate=true` does not weaken this rule; it only suppresses the routine problem-map approval step, not a genuine new-value or permission-denied question.
...
6. **Process-tree audit #1**: dispatch `process-tree-auditor` (`gpt-high`) against the Phase 4 subtree from `agents trace --json`. A `blocking` verdict halts the pipeline until the affected subtree is rerun.
...
Classify both trigger sources per `~/ai/conventions/agent-questions-and-session-graph.md`: a sub-agent emits `NEEDS_INPUT:<question_artifact_path>`, or this orchestrator hits `AskUserQuestion` permission-denial on a value/scope question.
