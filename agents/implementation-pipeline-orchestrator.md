...
4. **All four must return LOW** before proceeding. For a current, well-formed semantic supported-surface `MEDIUM` or `HIGH`, run a strategy-selection sub-step before generic revise:
   - Classify each supported-surface finding as exactly one of `in_domain_current_wu`, `unrelated_caller_domain`, or `needs_value_input`; missing, malformed, unreadable, blank, or stale supported-surface evidence stays on the existing evidence-repair path and is not strategy-selectable.
   - `in_domain_current_wu`: apply In-WU head-on remediation, record `STRATEGY_PHASE4_SUPPORTED_SURFACE_INWU` in `${planning_dir}/audit-history.md` role outputs / decision-register fields, then re-dispatch all four current proposal-risk gates from clean state. Old `LOW` reports are discarded.
   - `unrelated_caller_domain`: file a follow-up ticket decomposition artifact through `${ticket_operator}`, record `STRATEGY_PHASE4_SUPPORTED_SURFACE_FOLLOWUP`, then either narrow the current WU and re-dispatch all four proposal-risk gates from clean state or return explicit `DECOMPOSED`.
   - `needs_value_input`: write `${scratch_dir}/questions/q-<uuidv4>.question.json` and halt with `NEEDS_INPUT:<absolute_question_artifact_path>` per `~/ai/conventions/agent-questions-and-session-graph.md`.
   Any other current semantic `MEDIUM` or `HIGH` triggers a revise loop:
...
5. Run the Phase 4 code-quality gate below. When the Phase 4 code-quality aggregate verdict is `MEDIUM` or `HIGH`, also run the Phase 4 bootstrap-exception sub-gate below before writing the Phase 4 join manifest; only that sub-gate may authorize advance past a non-LOW Phase 4 code-quality aggregate.
6. **Write Phase 4 join manifest.** After all four current risk reports are LOW and the current Phase 4 code-quality aggregate passes (either as `LOW` or as a non-LOW aggregate ratified by the Phase 4 bootstrap-exception sub-gate below), and before applying supported-surface termination, write `${planning_dir}/risk/phase-4-join-manifest.json`. The manifest records one object per gate (`audit`, `scope`, `shortcut`, `supported-surface`) plus one object for `code-quality`, each with `gate_name`, `producing_invocation_uuid`, `canonical_output_path`, `size`, `mtime`, `sha256`, `verdict_line`, and `verified_at`. When the Phase 4 bootstrap-exception sub-gate fired and ratified the non-LOW code-quality aggregate, the manifest ALSO records one additional optional object `gate_name=bootstrap-exception` carrying `verdict_line=RATIFIED`, `ratifies_gate=code-quality`, `allow_advance_basis=bootstrap-exception`, plus the same identity/integrity fields (`producing_invocation_uuid`, `canonical_output_path` pointing at `${planning_dir}/proposals/${wu_lower}-${wu_id}.md`, `size`, `mtime`, `sha256`, and `verified_at`). The `bootstrap-exception` row's `verdict_line=RATIFIED` does not signal `LOW`; the code-quality row retains its actual non-LOW verdict beside it. For proposal-risk rows, `canonical_output_path` is the expected `${planning_dir}/risk/${wu_lower}-{gate}.md` path. For the code-quality row, `canonical_output_path` is `${planning_dir}/code-quality/${wu_lower}-phase-4/aggregate-code-quality.md`. The orchestrator reads `producing_invocation_uuid` from `agents trace --json` per the AGE-144 v1 invocation-output manifest schema. That schema is declared at `conventions/agents-trace-output.md` in the agent-runner repository's repo-root (the file lives in the separate agent-runner repo, NOT under `~/ai/`; do not expect a `/home/nes/ai/conventions/agents-trace-output.md` file — the canonical common-interface schema for `agents trace --json` consumers is owned by agent-runner). Per the v1 schema's `## Schema future / deferred` section, v1 consumers performing output path-to-invocation joins MUST use topology plus canonical-output-path matching; the orchestrator implements this v1 consumer contract by selecting the UUID of the most recent completed direct-child invocation (`completion_status=succeeded` per the v1 `## Field catalog` enum which declares `invocation_uuid`, `parent_invocation_uuid`, `completion_status`, `completion_timestamp`) whose declared output matches the gate's canonical report path. If a single most recent producing invocation cannot be unambiguously identified, the orchestrator blocks before writing the manifest and treats the condition as a missing phase artifact under the Violation Detection and Escalation policy. The orchestrator computes `size`, `mtime`, and `sha256` from that path on disk, and parses `verdict_line` from the canonical path on disk; verdict evidence is not trusted from stdout, `WROTE:`, or agents-result JSON.
7. Apply the supported-surface termination rule: invalidated-assumption → return to research; non-positive value → terminate the WU and emit a NEEDS_INPUT new-value-question to the root.
8. **Process-tree audit #1**: dispatch `process-tree-auditor` (`gpt-high`) against the Phase 4 subtree from `agents trace --json`. The expected-process manifest names the entry-mode subtree when Phase 3 consumed `review_first` or `plug_existing_review` evidence, so the Phase 4 audit can verify both the proposal-risk fanout, the Phase 4 code-quality gate, and the pre-proposal audit fanout lineage. For Phase 4 canonical rows, compose `expected_process` from `${planning_dir}/risk/phase-4-join-manifest.json`: copy `canonical_output_path`, copy `sha256` to `expected_sha256`, and supply `expected_verdict` from the owning gate contract (`LOW` for proposal-risk rows; `LOW` for the code-quality row), not from `verdict_line`. When the manifest also carries a `bootstrap-exception` row, the expected-process composition takes that row's `verdict_line=RATIFIED` as `expected_verdict` for that row, reads the actual non-LOW code-quality verdict beside it as evidence the ratification was applied, and the auditor MUST verify the `### <ticket-id> — Bootstrap exception ratification` entry exists in `${worktree_path}/DECISIONS.md` and cites `~/ai/conventions/code-quality.md` § `Bootstrap exception`; missing declaration, missing DECISIONS entry, missing convention citation, or absent `bootstrap-exception` manifest row beside a non-LOW code-quality verdict is a `blocking` finding. A `blocking` verdict halts the pipeline until the affected subtree is rerun.

#### Phase 4 code-quality gate

1. Trigger condition: after the four Phase 4 proposal-risk gates return current `LOW` and before writing `${planning_dir}/risk/phase-4-join-manifest.json`, run the composite code-quality workflow over the current WU evidence. This is a Phase 4 gate and does not replace the later per-component Phase 6 fanout, Phase 7 CodeRabbit, or Phase 8 PR-review gates.
2. Compose `${scratch_dir}/prompts/${wu_lower}-phase-4-code-quality.md`. The prompt instructs `~/ai/workflows/code-quality.md` to run in pipeline-callable mode with `slug=${wu_lower}-phase-4`, `repo_root=${repo_root}`, `worktree_path=${worktree_path}`, `planning_dir=${planning_dir}`, `scratch_dir=${scratch_dir}`, `wu_id=${ticket_id}`, `proposal_path=${planning_dir}/proposals/${wu_lower}-${wu_id}.md`, `problem_map_path=${planning_dir}/research/${wu_lower}-problem-map.md`, `risk_profile_path=${planning_dir}/risk/${wu_lower}-risk-profile.md`, `touched_surfaces_path=${planning_dir}/research/${wu_lower}-problem-map.md`, and any available current `diff_path`, `changed_files_path`, `changed_functions_path`, `base_ref`, `head_ref`, or `code_trace_paths`. The prompt MUST state that diff, changed-file, and changed-function evidence identifies touched files/components and evidence anchors; the audit set is the whole touched file/component under `~/ai/conventions/code-quality.md`, not only hunk lines or changed functions. When diff or function-change evidence is not available before implementation, the prompt must either supply equivalent proposal/touched-surface evidence accepted by the selected child auditor or mark the child inapplicable with a written manifest reason; it must not silently omit a relevant child.
3. Dispatch with the required ACR-151 shape: `agents -m gpt-high -p ${worktree_path} -f ${scratch_dir}/prompts/${wu_lower}-phase-4-code-quality.md 2>&1 | tee ${scratch_dir}/logs/${wu_lower}-phase-4-code-quality.log`.
4. Required artifact paths on disk: `${planning_dir}/code-quality/${wu_lower}-phase-4/dispatch-manifest.md`, `${planning_dir}/code-quality/${wu_lower}-phase-4/findings.json`, `${planning_dir}/code-quality/${wu_lower}-phase-4/findings.md`, `${planning_dir}/code-quality/${wu_lower}-phase-4/aggregate-code-quality.md`, and every report path marked `Required=true` in the dispatch manifest.
5. Verdict vocabulary check: parse `${planning_dir}/code-quality/${wu_lower}-phase-4/aggregate-code-quality.md` for `LOW`, `MEDIUM`, `HIGH`, `NEEDS_INPUT:<absolute_artifact_path>`, or `BLOCKED:<reason>`. `LOW` passes unconditionally. `NEEDS_INPUT` and `BLOCKED` are always blocking and are never ratifiable by the bootstrap-exception path. Missing manifest, missing required report, missing aggregate, unreadable, blank, malformed, stale, `NEEDS_INPUT`, and `BLOCKED` evidence are evidence-repair stop states; they do not enter strategy selection.
6. Strategy-selection sub-step for semantic non-LOW: when all required artifacts exist, are readable, non-blank, current, and well-formed, and the aggregate verdict is `MEDIUM` or `HIGH`, run the Phase 4 bootstrap-exception sub-gate below first if the proposal contains a bootstrap declaration. If that sub-gate emits a `bootstrap-exception` join-manifest row marked `RATIFIED`, advance under that existing local path only. Otherwise apply `~/ai/conventions/decomposition-strategies.md` before generic evidence-repair `NEEDS_INPUT`: choose In-WU head-on remediation and record `STRATEGY_PHASE4_CODE_QUALITY_INWU`, In-place helper extraction and record `STRATEGY_PHASE4_CODE_QUALITY_HELPER_EXTRACTION`, MOVE-and-import and record `STRATEGY_PHASE4_CODE_QUALITY_MOVE_AND_IMPORT`, In-place file-decomposition, or Follow-up ticket decomposition according to the current finding shape. After any in-WU, helper, MOVE, or file-decomposition action, rerun the Phase 4 code-quality gate before Phase 4 join-manifest publication. Follow-up ticket decomposition must narrow and rerun the current WU or return explicit `DECOMPOSED`; the ticket itself is not a pass state. The bootstrap exception remains local and unchanged; ACR-280 adds no second non-LOW allow-advance path.
7. Refusal-to-advance behavior: on missing manifest, missing required report, missing aggregate, unreadable/blank/malformed/stale required artifact, unresolved semantic `HIGH` or `MEDIUM` after bootstrap-exception fall-through and strategy-selection attempt, `NEEDS_INPUT:<absolute_artifact_path>`, or `BLOCKED:<reason>`, append `${planning_dir}/audit-history.md` with `actor=implementation-pipeline-orchestrator`, phase `Phase 4`, violation code `phase_4_code_quality_missing_or_blocking`, canonical paths, failed checks, and refused action `Phase 4 join manifest / supported-surface termination / Process-tree audit #1 / Phase 5 advance`. Write `${scratch_dir}/questions/q-<uuidv4>.question.json`, refuse Phase 4 join-manifest publication and Phase 5 advance, and halt with `NEEDS_INPUT:<absolute_question_artifact_path>` for evidence repair only when the failure is evidence repair or a genuine question artifact from a child. The orchestrator must not synthesize missing child reports, normalized findings, or aggregate findings inline.
8. Allow-advance condition: all required artifacts exist, are readable, non-blank, current for the Phase 4 proposal/problem-map/risk-profile/touched-surface evidence, and well-formed, and EITHER `aggregate-code-quality.md` reports `LOW` OR `aggregate-code-quality.md` reports `MEDIUM`/`HIGH` and the Phase 4 bootstrap-exception sub-gate below emitted a `bootstrap-exception` join-manifest row marked `RATIFIED`. The Phase 6 prototype-risk gate and the per-component code-quality auditor fanout retain their own existing verdict-disposition text and are unaffected by this rule; the bootstrap-exception path is local to the Phase 4 code-quality gate only.

#### Phase 4 bootstrap-exception sub-gate

This sub-gate is procedural Markdown: it parses evidence and routes the Phase 4 join manifest. It does NOT execute condition-by-condition truth-evaluation of the four bootstrap-exception conditions, does NOT recompute coupling/cohesion metrics, and does NOT run external scripts. The Phase 3 proposer is the source of truth for the four-condition argument per `~/ai/conventions/code-quality.md` § `Bootstrap exception`; this sub-gate verifies presence of declared evidence and a ratified DECISIONS citation, then emits a manifest row.

This sub-gate explicitly cites `~/ai/conventions/code-quality.md` § `Bootstrap exception` as the canonical authority for the four-condition exception and `~/ai/conventions/workflow-execution-violations.md` § `Named anti-pattern: Non-LOW gate residual acceptance` as the forbidden baseline. Silent advance with a non-LOW Phase 4 code-quality aggregate, advance on residual/advisory MEDIUM language, advance on manager-flavor or root acceptance language, and advance on a DECISIONS entry that lacks the canonical convention citation each remain a blocking workflow-execution violation; none of them are bootstrap-exception ratification.

1. Trigger condition: the Phase 4 code-quality aggregate verdict parsed at the Verdict-vocabulary-check step is `MEDIUM` or `HIGH` AND the approved Phase 3 proposal at `${planning_dir}/proposals/${wu_lower}-${wu_id}.md` contains a `## Bootstrap exception declaration` section. When the aggregate is `LOW`, the sub-gate does not run and no `bootstrap-exception` manifest row is emitted. When the aggregate is `MEDIUM`/`HIGH` but no Phase 3 declaration section exists, the sub-gate does not run and Phase 4 code-quality remains blocking under the LOW-only rule.
2. Parser step 1 — proposal evidence: re-open `${planning_dir}/proposals/${wu_lower}-${wu_id}.md` and parse the `## Bootstrap exception declaration` section. Verify every named field is present (parser work only, no truth-evaluation): `declared`, `code_quality_gate`, `measured_metric`, `expected_non_low_verdict`, `finding_ids`, `intrinsic_lockstep_paths`, `metric_change_refs`, `post_merge_new_rule_evidence`, `primary_deliverable_fixes_or_extends_metric`, `non_low_finding_is_intrinsic_lockstep`, `post_merge_satisfies_new_rule_under_new_metric`, `declared_for_phase_4_ratification`. Missing field → fall-through (step 5).
3. Parser step 2 — DECISIONS ratification evidence: parse `${worktree_path}/DECISIONS.md` for a heading matching `### <ticket-id> — Bootstrap exception ratification` where `<ticket-id>` equals `${ticket_id}` (case-insensitive on the ticket prefix). Verify the body of that entry cites `~/ai/conventions/code-quality.md` § `Bootstrap exception` as the canonical authority. Missing heading, malformed heading, or missing convention citation → fall-through (step 5).
4. Orchestration step — emit manifest row: when both Parser step 1 and Parser step 2 succeed, the orchestrator emits an additional Phase 4 join-manifest row with `gate_name=bootstrap-exception`, `verdict_line=RATIFIED`, `ratifies_gate=code-quality`, and `allow_advance_basis=bootstrap-exception`, alongside the existing `code-quality` row which retains its actual non-LOW verdict in its own `verdict_line` field. With both rows present, advance to Phase 5 is permitted; the join-manifest write step above is the canonical join point.
5. Fall-through: when Parser step 1 fails (missing proposal section or missing required field), when Parser step 2 fails (missing DECISIONS entry, missing convention citation, or malformed evidence), or when any required evidence is malformed, the sub-gate does NOT emit a `bootstrap-exception` row. The Phase 4 code-quality gate's `MEDIUM`/`HIGH` aggregate remains blocking under the LOW-only rule above and the Refusal-to-advance behavior fires unchanged.
6. Recording: the sub-gate's decision (emitted-row or fall-through) and the parsed evidence paths are referenced by `${planning_dir}/audit-history.md` and by the Phase 4 join manifest itself; no separate sub-gate artifact path is required, because the join-manifest row IS the orchestration record. The Process-tree audit #1 step above is responsible for verifying that the DECISIONS ratification entry continues to exist and continues to cite the convention when the manifest carries the row.
...
14. After Step 6c has produced passing level evidence and a candidate or accepted current-layer component inventory exists, detect whether coupling/cohesion evidence applies. Trigger condition: a `LevelComponentSet` exists in `${planning_dir}/contracts/${wu_lower}-${slug}.md` OR a candidate component inventory exists. WUs without either take the explicit non-applicability path in sub-step 19.
15. When the trigger fires, dispatch `~/ai/agents/coupling-auditor.md` (`gpt-high`) against current-layer component-pair integrations. Pass `repo_root`, `planning_dir`, `wu_id=${ticket_id}`, `proposal_path`, `problem_map_path`, `risk_profile_path`, and a `touched_surfaces_path` derived from the current component-pair list. The auditor writes `${planning_dir}/risk/${wu_lower}-coupling.md` per its default output path. Compose the dispatch prompt at `${scratch_dir}/prompts/${wu_lower}-phase-6c-coupling.md` and log to `${scratch_dir}/logs/${wu_lower}-phase-6c-coupling.log`.
16. When per-component organization evidence is needed, dispatch `~/ai/agents/cohesion-auditor.md` (`gpt-high`) against individual current-layer components. Inputs match the coupling auditor's contract; the auditor writes `${planning_dir}/risk/${wu_lower}-cohesion.md`. Compose prompt + log under the same `${scratch_dir}` paths with `-cohesion` suffix.
17. Record (or verify the code-writer recorded) a `CouplingDecision` adjacent to the current `LevelComponentSet` evidence inside `${planning_dir}/contracts/${wu_lower}-${slug}.md`, or adjacent to the candidate component inventory when no accepted `LevelComponentSet` exists. Required fields per the workflow doc: `level_id`, `source_level_component_set_ref`, `candidate_component_inventory_ref`, `coupling_audit_report_refs`, `cohesion_audit_report_refs`, `verdict`, `changed_component_refs`, `removed_or_retired_refs`, `new_abstraction_component_refs`, `integration_test_refs`, `rationale_evidence_refs`, `defer_to_prototype_risk`. The `verdict` value MUST be one of `merge_components`, `introduce_abstraction_component`, or `no_change`. Stay-at-current-level: `changed_component_refs` and `new_abstraction_component_refs` MUST stay at the current `level_id`; nested sub-components, grandchild components, and multi-layer hierarchy are forbidden in this decision (multi-layer hierarchy still routes through ACR-11's recursive child entry, not through this rule). The Step 6b output-index handoff field for this record is `coupling_decision_ref`; `introduce_abstraction_component` records an abstraction component at the current level.
18. When the verdict is `merge_components` or `introduce_abstraction_component` (i.e. inventory changes), verify `CouplingDecision.integration_test_refs` lists rerun/current integration tests covering the emitted current component inventory. Stale or missing refs trigger sub-step 19. The Phase 7 pre-dispatch integration-tests gate (existing) remains downstream and verifies `LevelComponentSet` shape + per-pair coverage separately — this sub-step is an additional currentness check, not a replacement.
19. On missing, unreadable, blank, incomplete, or invalid `CouplingDecision` evidence (or missing/invalid coupling/cohesion auditor reports when the trigger fired), append a violation entry to `${planning_dir}/audit-history.md` with `actor=implementation-pipeline-orchestrator`, phase `Phase 6`, violation code `coupling_decision_missing_or_invalid`, canonical path, refused action `Phase 6 advance to Process-tree audit #2 / Phase 7`, failed checks, and the question artifact path. Log the same violation to orchestrator stderr. Refuse advance to Process-tree audit #2 / Phase 7. Write `${scratch_dir}/questions/q-<uuidv4>.question.json`. Halt with `NEEDS_INPUT:<absolute_question_artifact_path>`. This is a blocking `NEEDS_INPUT` for evidence repair, not a self-resolvable procedural question; the orchestrator must not generate or supply the missing artifact. **Non-applicability path:** when the sub-step 14 trigger does not fire (no `LevelComponentSet` and no candidate component inventory exists), require an explicit `non-applicable` statement at the canonical `CouplingDecision` adjacency locator inside `${planning_dir}/contracts/${wu_lower}-${slug}.md`. A missing artifact is NOT equivalent to non-applicability — this mirrors the Phase 6 halt-state transition gate and the Phase 7 swap-record gate.
...
4. Verdict vocabulary check: parse `aggregate-code-quality.md` for completed severity `LOW`, `MEDIUM`, or `HIGH`, or preserved stop state `NEEDS_INPUT:<absolute_artifact_path>` or `BLOCKED:<reason>`. `LOW` passes. `MEDIUM`, `HIGH`, `NEEDS_INPUT`, and `BLOCKED` are blocking for per-component code-quality fanout; stable-MEDIUM allowance belongs only to gates with explicit prototype-risk authority, not to pipeline-callable code-quality.
...
6. Allow-advance condition: all required artifacts exist, are readable, non-blank, current, and well-formed, and `aggregate-code-quality.md` reports `LOW`. This fanout does not replace downstream PR-review aggregate gates.
...
Before dispatching `coderabbit-operator`, enforce inherited prototype proof-test readiness from `~/ai/workflows/implementation-pipeline.md` and `~/ai/conventions/prototype-pending-tests.md` § `Carry-forward to implementation`.
...
3. Compose `${scratch_dir}/prompts/${wu_lower}-phase-8-code-quality.md` for actual PR diff code-quality review, set `prompt=${scratch_dir}/prompts/${wu_lower}-phase-8-code-quality.md` and `log=${scratch_dir}/logs/${wu_lower}-phase-8-code-quality.log`, then dispatch Phase 8 code-quality after the four PR-review gates and before writing `phase-8-join-manifest.json`: `agents -m gpt-high -p ${worktree_path} -f ${prompt} 2>&1 | tee ${log}`. Require canonical outputs under `${planning_dir}/code-quality/${wu_lower}-phase-8/`: `dispatch-manifest.md`, `findings.json`, `findings.md`, child reports, and `aggregate-code-quality.md`.
4. Parse `${planning_dir}/code-quality/${wu_lower}-phase-8/aggregate-code-quality.md` for `LOW`, `MEDIUM`, `HIGH`, `NEEDS_INPUT:<absolute_artifact_path>`, or `BLOCKED:<reason>`. Missing, unreadable, blank, malformed, stale, `NEEDS_INPUT`, and `BLOCKED` evidence block under the existing evidence-repair path. For current, well-formed semantic Phase 8 code-quality `MEDIUM` or `HIGH`, run the strategy-selection sub-step from `~/ai/conventions/decomposition-strategies.md` before split handling, Process-tree audit #3, or PR creation:
   - MOVE-and-import is the default when WU code lifts cleanly from a touched god-file; record `STRATEGY_PHASE8_CODE_QUALITY_MOVE_AND_IMPORT`.
   - In-place file-decomposition applies only for one bolted-on domain; record `STRATEGY_PHASE8_CODE_QUALITY_FILE_DECOMPOSITION`.
   - In-WU remediation or helper extraction applies for WU-authored code/test findings, including WU-authored test debt after MOVE; record `STRATEGY_PHASE8_CODE_QUALITY_INWU`.
   - Follow-up-ticket decomposition applies for unrelated multi-domain touched-file debt after current-WU narrowing; record `STRATEGY_PHASE8_CODE_QUALITY_FOLLOWUP`.
   After any MOVE, file split, helper extraction, or in-WU remediation, rerun Phase 8 code-quality and require current `LOW` before Process-tree audit #3 and PR creation. Follow-up tickets remain decomposition artifacts: they require current-WU narrowing and rerun or explicit `DECOMPOSED`, never allow advance by themselves.
5. **Write Phase 8 join manifest.** Write `${planning_dir}/risk/phase-8-join-manifest.json` before split handling or Process-tree audit #3. The manifest records one object per PR-review gate and a fifth `code-quality` row with `gate_name=code-quality`, `canonical_output_path=${planning_dir}/code-quality/${wu_lower}-phase-8/aggregate-code-quality.md`, and the same identity/integrity fields as PR-review rows: `gate_name`, `producing_invocation_uuid`, `canonical_output_path`, `size`, `mtime`, `sha256`, `verdict_line`, and `verified_at`. The orchestrator reads `producing_invocation_uuid` from `agents trace --json` per the AGE-144 v1 invocation-output manifest schema. That schema is declared at `conventions/agents-trace-output.md` in the agent-runner repository's repo-root (the file lives in the separate agent-runner repo, NOT under `~/ai/`; do not expect a `/home/nes/ai/conventions/agents-trace-output.md` file — the canonical common-interface schema for `agents trace --json` consumers is owned by agent-runner). Per the v1 schema's `## Schema future / deferred` section, v1 consumers performing output path-to-invocation joins MUST use topology plus canonical-output-path matching; the orchestrator implements this v1 consumer contract by selecting the UUID of the most recent completed direct-child invocation (`completion_status=succeeded` per the v1 `## Field catalog` enum which declares `invocation_uuid`, `parent_invocation_uuid`, `completion_status`, `completion_timestamp`) whose declared output matches the gate's canonical report path. If a single most recent producing invocation cannot be unambiguously identified, the orchestrator blocks before writing the manifest and treats the condition as a missing phase artifact under the Violation Detection and Escalation policy. The orchestrator computes `size`, `mtime`, and `sha256` from `canonical_output_path`, and parses `verdict_line` from the canonical path on disk; verdict evidence is not trusted from stdout, `WROTE:`, or agents-result JSON.
