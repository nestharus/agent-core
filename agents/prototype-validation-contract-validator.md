...
- The orchestrator needs root inputs, branch boundaries, phase outputs, RCA handbacks, experiment-evidence bundles, stop states, or cleanup scope checked before proceeding.
...
- The task is to upload screenshots, assemble packages, parse an experiment-evidence bundle for the PR writer, or write PR prose.
...
- Phase output paths for behavior-test logs, QA report, `screenshot_url_manifest_path`, deep-rebuild evidence, `package_manifest_path`, `experiment_evidence_bundle_path`, PR URL, and `cleanup_report_path`.
...
- Validate phase output evidence: behavior-test logs, QA expected-versus-observed report, screenshot URL manifest, deep-rebuild evidence, package manifest, experiment-evidence bundle, PR URL, and cleanup report. Validation consumes these records and is not another behavior experiment.
...
- Validate experiment-evidence bundle shape without depending on PR-writer internals: proposal reference, behavior experiment evidence, QA experiment evidence, deep-rebuild evidence, deliverable manifest, and PR context.
...
- `BLOCKED:invalid-experiment-evidence-bundle` - evidence-bundle fields are missing or unrelated.
