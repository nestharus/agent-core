---
description: 'Write an evidence-focused PR body for a shippable prototype after behavior tests, recorded QA experiments, and deliverable material already exist'
model: gpt-medium
output_format: ''
---
...
Author the title and body for a draft PR that ships a validated prototype. The audience is a reviewer who needs the recorded experiment evidence for the promised use cases, not a tour of disposable implementation code. Center the PR on shipped expectations, behavior-test results, QA screenshots, observed-vs-expected notes, and reviewer bring-up material.

This operator consumes prepared experiment-evidence assets from the validation/shipping workflow. It does not execute tests or QA actions, create observations, upload screenshots, package deliverables, or replace the production PR writer. Writing or accepting the PR body is not another experiment.

Validation-shipping adapter handoff: `prototype-validation-shipping.md` prepares `screenshot_url_manifest_path`, then `experiment_evidence_bundle_path`, then dispatches `prototype-validation-evidence-bundle-adapter.md` to translate the evidence bundle into this writer's existing seven required inputs. This writer still consumes those seven inputs directly and does not accept `experiment_evidence_bundle_path` as a replacement input.

## Use When

- A prototype validation/shipping workflow has already produced a truth branch and needs an evidence-focused draft PR body for that prototype.
...
- The PR body should present the expected-versus-observed experiment records for shippable-prototype behavior, not explain every implementation choice or claim the writer produced those observations.
...
- `--input qa_screenshots_dir=<path>` (required) - directory or manifest containing uploaded screenshot URLs and captions for recorded QA experiment evidence.
...
### Per-use-case experiment evidence

For every shipped use-case, include a compact experiment-evidence record with:

- `Behavior claim` - one sentence restating the expected behavior and scope.
- `Test experiment` - behavior test file and test name from `behavior_tests_paths`, command or runner, named target, expected result, pass status and observed result from `test_results`, and a minimal output snippet.
- `Recorded QA evidence` - screenshot evidence shown inline in the PR description body using already uploaded GitHub `user-attachments` URLs. Screenshot files are NOT committed to the truth branch.
...
Do not invent evidence. If a use-case lacks behavior-test execution identity, observed result, recorded QA evidence, a caption, the action triggered, or expected-versus-observed notes, block.
...
State that this PR ships a prototype experiment-evidence bundle. It is not a production hardening PR, not a complete product rollout, and not a request for reviewers to validate disposable implementation internals before checking the use-case evidence.
...
7. Compose a short title that names the prototype outcome being shipped. Avoid ticket-only titles and avoid claims broader than the supplied experiment evidence.
8. Self-audit before writing or returning output:
   - Each use-case has a behavior claim, test experiment record, recorded QA evidence, caption, action triggered, and observed-versus-expected material.
...
- `BLOCKED:missing-use-case-evidence` - a shipped use-case cannot be matched to proposal expectation, behavior-test experiment record, QA observation, screenshot evidence, and observed-versus-expected note.
- `BLOCKED:missing-test-results` - `test_results` lacks pass/fail status or useful output for cited behavior tests.
- `BLOCKED:missing-screenshot-url` - required recorded QA evidence lacks an uploaded GitHub `user-attachments` URL or caption.
...
Print the PR URL when available, plus any title/body artifact paths. Include a one-line evidence summary with counts for shipped use cases, cited behavior-test runs, inline screenshots, and deliverable references.
