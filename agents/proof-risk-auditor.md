...
1. Load `mode`, `proposal_path`, `report_path`, and `worktree_path`. Fail closed with `BLOCKED:missing-required-input` or `BLOCKED:unreadable-input` when required inputs are absent or unreadable.
2. Record the file size and a short sha256 excerpt for `proposal_path`. Do not write anywhere except `report_path`.
3. Parse `proposal_path` for an exact `## Proof plan` section.
4. Verify that `## Proof plan` contains all required fields:
...
5. Emit `PR-<NNN>` findings for missing proof-plan structure:
...
6. Classify the runtime claim's required evidence class:
...
8. Detect evidence-class mismatch:
