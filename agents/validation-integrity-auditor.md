...
2. Record the file size and a short sha256 excerpt for every input artifact read. Resolve relative evidence paths against `worktree_path`; do not read outside supplied artifacts except for path resolution.
3. Parse the diff or dossier diff by hunks. Treat removed lines, added lines, and adjacent unchanged context as evidence anchors.
4. Detect validation-weakening patterns:
...
5. Treat "tests now pass" as insufficient when any validation surface changed in the same diff or dossier and the runtime claim is artifact-bound. Classify it through the concrete pattern that changed the validation surface, or through `VI-007` when the only proof surface is test-environment evidence.
6. For each fired pattern, evaluate ratification only when both `decisions_path` and `runtime_artifact_evidence_path` are supplied:
...
7. Emit no finding for pure renames, helper extraction, formatting, or equivalent refactors that preserve the same runtime condition and do not substitute proxy proof for the runtime claim.
8. Emit no finding for proxy evidence that is honestly scoped to a proxy-layer claim while the runtime claim has separate runtime-artifact evidence.
9. Assign report-local IDs as `VI-001`, `VI-002`, ... by finding order. If `wu_id` is supplied, you may additionally show `VI-<wu_id>-<NNN>` inside the report, but the terminal verdict stays only the verdict token.
10. Write the report to `report_path` and return the terminal verdict on the last non-blank line of the report and on stdout.
