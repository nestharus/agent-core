...
3. Record the file size and a short sha256 excerpt for every input artifact read. Resolve relative evidence paths against `worktree_path`; do not assume the current working directory is the worktree. Before pattern detection, when `runtime_artifact_evidence_path` is supplied, read and validate it as an executed experiment record under `conventions/behavioral-proof.md` Evidence Integrity: it must be readable and non-empty, identify the command or action and runtime target, compare expected and observed results, record status and relevant output or retained observation, retain provenance, and reference the runtime artifact named by `runtime_claim`. Treat a missing, unreadable, empty, artifact-mismatched, or proxy-only record as no valid runtime-artifact evidence; an invalid optional evidence path does not block the audit.
