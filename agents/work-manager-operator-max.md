...
| Phase 7 CodeRabbit finding | Resolve every finding through its independent conversation, wait for CodeRabbit to resolve every GraphQL thread, and require CodeRabbit approval on the exact final head before Phase 8. Pushback continues only that conversation; never request a follow-up review generation. |
