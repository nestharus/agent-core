...
- Reviewing test quality, CodeRabbit finding resolution, PR diff justification, or commit hygiene. Use the existing gates.
...
6. Inspect the operator under audit for an optimized contract sidecar at `contracts/operators/<operator>.yaml` or a `## Contract` section containing one fenced YAML block with `schema: operator-contract-v1`. First classify the target as `new-operator`, `project-wrapper`, `edited-high-risk`, or `trivial-minimum-body`:
   ...
   Emit `severity: HIGH` blocking findings for `new-operator`, `project-wrapper`, and `edited-high-risk` when the contract surface is missing or malformed:
   - Missing contract sidecar and `## Contract` section: `severity: HIGH`, `class: missing-contract-surface`.
   - Present contract surface but YAML fails to parse: `severity: HIGH`, `class: malformed-contract-yaml`.
   ...
   The high-risk classification heuristic must match the regression fixture shape in `evals/acr-279-lint-promotion-regression/fixtures/high-risk-operator-missing-contract.md`: high-risk ticket/operator signals plus no contract surface must fail the curated gate with a blocking finding.
...
- Contract-surface findings are blocking for promoted categories. Missing or malformed contract sidecars/blocks and schema mismatches are HIGH for new operators under `~/ai/agents/`, project wrappers under `~/projects/<*>/agents/`, and edited high-risk operators touching external services, credentials, branch topology, releases, PRs, tickets, or worktrees.
- Contract-surface findings remain advisory for trivial/minimum-body operators outside those promoted categories per `operator-file-format.md` § `Minimum Body`, unless the target also has concrete high-risk dispatch, credential, branch, release, PR, ticket, or worktree behavior.
- Missing rich recommended body sections are still not contract-surface failures. They become MEDIUM/HIGH only when they create routing ambiguity, unsafe invocation, unclear output, or unbounded scope.
