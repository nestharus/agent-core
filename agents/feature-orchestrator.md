...
inputs:
  ...
  - name: local_coverage_command
    ...
    required: true
    default_source: caller
    description: "Non-blank project coverage command passed unchanged to every direct or refactoring-nested implementation child; its exact SHA-256 binds route dispatch and replay evidence, and missing or blank input blocks before feature worktree creation or route side effects with BLOCKED:missing-local-coverage-command."
...
`local_coverage_command` is a required caller-owned non-blank string because both supported feature routes reach implementation Phase 8. Validate it before feature worktree creation or any route side effect, preserve the exact bytes for child input transport, and compute `local_coverage_command_sha256=SHA256(UTF-8(local_coverage_command))` without trimming or normalization. Missing or whitespace-only input is `BLOCKED:missing-local-coverage-command`. The command is execution configuration: never place it in `route_payload`, `evidence_pack_context`, ambient configuration, or free-form anti-scope prose.
...
normalized_output_coverage_binding: local_coverage_command_sha256
...
3. Require one record per scoped ticket and exact set equality with `scoped_ticket_list`; reject missing or extra tickets. Require each ticket source to contain exactly the existing `jira_issue_key` or `linear_issue_key` selected by `ticket_system`, require that provider issue key to equal immutable graph `ticket_id`, and reject `wu_brief_path`. Keep `local_coverage_command` outside every route record and bind only its exact SHA-256 at the normalized manifest top level.
...
- `${planning_dir}/route-manifest.json`: source identity/hash, exact `local_coverage_command_sha256`, explicit branches, backend identity, normalized route records, unique derived route paths, deterministic topological order, and immutable ready waves.
...
additional_properties: false
identity_fields: [feature_branch, local_coverage_command_sha256, ticket_id, attempt_number, owning_route]
...
1. Derive and validate `feature_invocation_uuid`, validate the canonical invocation including non-blank `local_coverage_command`, compute its exact SHA-256, then parse and validate the complete discriminated route source under `feature-route-source-v2`. Write the normalized route manifest only after every source and derived identity passes. This validation and hash capture precede feature worktree creation and every route side effect.
...
Dispatch `implementation-pipeline-orchestrator` with the record's exact existing `jira_issue_key` or `linear_issue_key`; `ticket_system`; matching backend configuration; `repo_root`; `worktree_path=${route_worktree_path}`; `planning_dir=${route_planning_dir}`; `scratch_dir=${route_scratch_dir}`; `branch_name`; `base_branch=${feature_branch}`; exact `route_attempt_number=${attempt_number}`; exact `local_coverage_command=${local_coverage_command}`; and `auto_merge_after_phase_9=false`. Put the command only in the resolved child's declared input block and preserve it byte-for-byte; do not restate it as free-form prompt prose. Pass both normalized roots unchanged: `route_planning_dir.parent` is the child's declared live runtime root `F/routes` and owns exactly `F/routes/sessions.active-wake.json`, while `route_scratch_dir` remains separately normalized under the same top-level project planning tree. Later wake composition must be invoked explicitly with `planning_root=F/routes`, which binds only `F/routes/sessions.active-wake.json`; it must not select another index, and this route does not create or discover a project-global row. The issue key equals immutable manifest `ticket_id`, so a feature route never dispatches `wu_brief_path`, never enters Phase 0 ticket creation, and cannot replace graph identity with a newly returned provider key. Never permit the child default to select merge ownership.
...
Dispatch `refactoring-orchestrator` with that same-named sole existing-issue ticket-source field; `ticket_system`; matching backend configuration; validated brief-plus-surfaces `target_list` and title/surfaces `slice_bounds` from `route_payload`; `repo_root`; already-normalized `branch_name`; `worktree_path=${route_worktree_path}`; `planning_dir=${route_planning_dir}`; `scratch_dir=${route_scratch_dir}`; `feature_routed=true`; exact `local_coverage_command=${local_coverage_command}`; `trunk_branch=${trunk_branch}`; `integration_branch_ref=${feature_branch}`; canonical `protected_branches=[${trunk_branch},${feature_branch}]`; and `manager_flavor`. Put the command only in the resolved refactoring input block; that owner passes the same value unchanged through its declared nested implementation input. The issue key equals immutable manifest `ticket_id`, so the nested implementation child reads the existing issue and cannot enter Phase 0 ticket creation. Do not put a child invocation UUID in the prompt. After dispatch, join the captured marker UUID to the returned runtime-derived `refactoring_invocation_uuid`. Missing adapter data is `BLOCKED:missing-route-input` and must have been rejected before normalization.
...
Freeze the `feature-route-attempt-outcome-v1` and common validation result with exact `feature_branch`, then freeze one `feature-route-attempt-proof-v1` with the same branch identity, exact `local_coverage_command_sha256`, and current path/SHA-256 references for prompt, log, child result, route evidence, both stage triples, auditor prompt/log/output/report/binding, all child-owned proof companions, common validation, and route-specific outcome evidence. `validate-route-attempts` compares that hash with the current normalized manifest, so replay under any byte-different command fails closed for both routes. For a direct implementation attempt, only then may the feature owner create the exact-feature-branch pre-ready currentness and implementation-only acceptance envelope; direct merge authorization composes common proof with caller-bound ticket evidence, currentness, acceptance, and fresh non-draft validation, preventing circular hashes. A refactoring accepted result instead composes common proof with the route result's exact feature branch/ref names, verified merge/base/head/final-integration identity, and ancestry/parent PASS. A stale or replay-required attempt remains immutable and permits only a new numbered attempt through the same owner.
...
- Stop before feature worktree creation or route side effects with `BLOCKED:missing-local-coverage-command` when `local_coverage_command` is absent or blank.
